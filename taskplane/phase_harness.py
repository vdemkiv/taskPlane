...
def initialize(runtime: Any, workspace: str, state: dict[str, Any]) -> None:
    ...
    configuration = {
        ...
        "knowledge_reference": artifacts.put("phase-knowledge", {
            "schema": "taskplane.scoped-knowledge/v1", "facts": [],
        ...
        "host_version": os.environ.get("TASKPLANE_HOST_VERSION") or
                        os.environ.get("CLAUDE_CODE_VERSION") or "unknown",
        "output_paths": {
            "product": {"requirement": "specs/requirement.json"},
            "design": {"design": "design/contract.json",
    ...
    def authorize(current: Any) -> None:
        ...
        if runtime._stage_native_init_authority(workspace, state["requirement_id"], authority["actor"]) != authority:
            raise ValueError("new phase route authority changed")
    phase_records.change_phase_routing(store, state["run_id"], owner="agent-runtime",
        configuration=configuration, expected_previous=None,
        expected_revision=manifest["revision"], operation_id="initialize-phase-runtime",
...
def store_input(context: dict[str, Any], action: dict[str, Any]) -> dict[str, Any]:
    ...
    inputs = {
        "schema": "taskplane.phase-input/v1", "run_id": context["run_id"],
        ...
        "requirement": action["requirement"], "outputs": action["phase_outputs"],
        ...
        **({"accepted_evaluations": action["accepted_evaluations"]}
           if "accepted_evaluations" in action else {}),
        **({"graph_conformance": action["graph_conformance"]}
           if "graph_conformance" in action else {}),
        ...
        **({"graph_baseline": action["graph_baseline"]} if "graph_baseline" in action else {}),
        "artifacts": [{**row, "reference": review_evidence.portable_artifact_reference(store, row["reference"])}
    ...
    for output in inputs["outputs"]:
        path = output["path"]
        if path is not None and (os.path.isabs(path) or re.match(r"^[A-Za-z]:", path)
...
def dispatch_action(runtime: Any, workspace: str, context: dict[str, Any], action: dict[str, Any]) -> dict[str, Any]:
    ...
    host_keys = {"step", "role", "role_marker", "task_name", "model_tier", "model",
                 "reasoning_effort", "dispatch_route", "settings_digest",
                 "dispatch_intent", "wait_policy", "contract_bootstrap"}
    ...
    if action.get("evidence_children"):
        obligations["children"] = [{"schema": "taskplane.stage-dispatch/v1",
            "stage_runtime_dispatch": runtime.tp.attach_phase_input(
                action["stage_runtime_dispatch"], child["evidence_input"]),
            "obligations": {**{key: child[key] for key in host_keys if key in child},
                            "dispatch_allowed": True}}
            for child in action["evidence_children"]]
    return {"schema": "taskplane.stage-dispatch/v1", "stage_runtime_dispatch": envelope,
...
def read_input(runtime: Any, workspace: str, envelope: dict[str, Any]) -> dict[str, Any]:
    ...
    if slot is not None:
        contract = runtime.tp.load_json(runtime.tp.active_contract_path(workspace, slot), default=None)
        if isinstance(contract, dict) and contract.get("evidence_input") == startup["phase_input"]:
            ...
            if value.get("schema") != "taskplane.evaluate-child-input/v1" or any(
                    value.get(key) != expected for key, expected in {
                        "stage_id": context["stage"]["stage_id"], "run_id": context["run_id"],
                        "authority_fingerprint": context["stage"]["authority"]["authority_fingerprint"]}.items()):
                raise ValueError("evidence input belongs to different authority")
            return cast(dict[str, Any], value)
        if not isinstance(contract, dict) or (contract.get("phase_runtime") or {}).get("reference") != row["result"]["reference"]:
    ...
    value = store.read(startup["phase_input"])
    if value.get("schema") != "taskplane.phase-input/v1" or any(value.get(key) != expected for key, expected in {
            "stage_id": context["stage"]["stage_id"], "run_id": context["run_id"],
            "authority_fingerprint": context["stage"]["authority"]["authority_fingerprint"]}.items()):
...
def accepted_evaluations(runtime: Any, context: dict[str, Any], state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for task in state.get("tasks") or []:
        ...
        if result["run_id"] != context["run_id"] or result["phase_id"] != "evaluate" or result["status"] != "accepted":
            raise ValueError("Engineering task collection is foreign or unaccepted")
        references = [ref for ref in result["collected_output_references"] if ref["kind"] == "judgment"]
        if len(references) != 1:
        ...
        accepted.append({"task_id": task["id"], "candidate_sha": task["target_commit"],
            **{key: review_evidence.portable_artifact_reference(store, reference)
               for key, reference in {"judgment": references[0], "handoff": completion["handoff"],
                   "runtime_receipt": completion["runtime_receipt"]}.items()}})
        lens_refs = [ref for ref in result["collected_output_references"] if ref["kind"] == "lens-evidence"]
        if len(lens_refs) != 1:
            raise ValueError("Engineering task lacks its complete lens evidence package")
        accepted[-1]["lens_evidence"] = review_evidence.portable_artifact_reference(store, lens_refs[0])
    if not accepted:
...
def prepare_lenses(runtime: Any, context: Any, material: dict[str, Any], worker_input: dict[str, Any],
    ...
    inputs = {row["artifact_class"]: store.read(row["reference"])
              for row in material["package"] if row["artifact_class"] != "lens-evidence"}
    ...
    if selected:
        policy = runtime.operational_settings.load_settings(
            environment=runtime.os.environ).lenses.policy_for(definition["id"],
                catalog_ids={row["id"] for row in lens.load_catalog()["lenses"]})
        _, routing, request = runtime._focused_stage_route(store.workspace,
            stage=definition["id"], target=binding["candidate_fingerprint"],
        ...
        routing = {"lenses": [{**row, "tier": "n/a", "negative_evidence": [
    ...
    envelope = review_evidence.create_envelope(store,
        ...
        diff={"files": files, "phase_inputs": inputs}, impact=impact, graph_quality=graph_quality,
...
def phase_candidates(runtime: Any, workspace: str, material: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact_class, path in material["output_paths"].items():
        ...
        raw = runtime._stage_loop_read_output_no_follow(output_root, relative, required=True,
...
def collect_lenses(runtime: Any, workspace: str, envelope: dict[str, Any], *, prepare: bool = False) -> dict[str, Any]:
    ...
    row = phase_records.phase_records(context["store"].load(context["run_id"]))[operation_id(context)]
    ...
    if prepare:
        bound = runtime._bind_stateless_review_contract_actions(workspace,
            {"status": "ready", "run_id": context["run_id"],
             "slots": context["artifacts"].read(plan)["dispatch"]},
            task_id=material["bindings"]["operation_id"])
        return {"plan": plan, "dispatch": bound["slots"],
...
def lens_evidence(store: Any, material: dict[str, Any]) -> dict[str, Any]:
    ...
    for reference in references:
        ...
        for entry in packet["entries"]:
            ...
            if plan["binding"]["run_id"] != material["bindings"]["run_id"] or collection["status"] != "complete":
...
def lens_gate(runtime: Any, workspace: str, state: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for reference in result["collected_output_references"]:
        ...
        for entry in store.read(reference)["entries"]:
            collection = store.read(entry["collection"])
            failed = [verdict["lens"] for row in collection["results"]
                      for verdict in row["lens_results"] if verdict["verdict"] == "fail"]
            if failed:
                return {"error": "selected lens findings require changes: " + ", ".join(failed),
...
def _initial_operation(context: dict[str, Any]) -> str:
    prepared = sorted((row for row in phase_records.phase_records(context["manifest"]).values()
...
def retry_chain(context: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    rows = sorted((row for row in phase_records.phase_records(context["manifest"]).values()
...
def advise_resource_limits(runtime: Any, ws: str, state: dict[str, Any], by: str) -> dict[str, Any]:
    ...
    value = {"schema": "taskplane.resource-policy/v1", "run_id": context["run_id"],
        ...
        "authority_fingerprint": context["stage"]["authority"]["authority_fingerprint"]}
    phase_records.commit_phase_record(context["store"], context["run_id"],
...
def _record_usage_snapshot(runtime: Any, ws: Any, lifecycle: Any, snapshot: Any) -> None:
    ...
    runtime.record_observed_dispatch_usage(ws, task_id=task_id, dispatch_id=dispatch_id,
        native_task_name=lifecycle["expected_task_name"], source_fingerprint=snapshot["source"]["path_fingerprint"],
...
def resolve_retry(runtime: Any, ws: str, state: dict[str, Any], *, operation: str, candidate: str,
    ...
    if prior is not None:
        ...
        if candidate != config["candidate_fingerprint"]:
            ...
            phase_records.change_phase_routing(context["store"], context["run_id"],
    ...
    route = phase_records.change_phase_routing(context["store"], context["run_id"],
    ...
    receipt = phase_records.commit_phase_record(context["store"], context["run_id"],
...
def pending(runtime: Any, ws: str, state: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    if completed is None:
        nonce = runtime.design_host_transport.phase_nonce_source(runtime.tp, ws, context["run_id"], existing_only=True)
...
def _phase_bridge_registry(_ports: Any, config: Mapping[str, Any]) -> Any:
    ...
    raw = _ports._stage_loop_read_output_no_follow(root, source, required=True, remaining_bytes=1024 * 1024)
    data = _ports.json.loads(raw)
    rows = data
    registered = {
        "taskplane.loop.validate_spec_phase_artifact": ("spec-phase/v1", _ports.validate_spec_phase_artifact),
    ...
    skills = {row["skill_ref"]: _ports._stage_loop_read_output_no_follow(root, row["skill_ref"],
        required=True, remaining_bytes=1024 * 1024) for row in rows}
    registry = _ports.operational_settings.load_phase_registry(rows, skills=skills,
        validator_inventory=inventory, artifact_schemas=schemas)
    if (config.get("definition_set_fingerprint") is not None and
            registry.definition_set_fingerprint != config["definition_set_fingerprint"]):
        raise ValueError("phase definition set changed")
    return registry, {name: registered[name][1] for name in ids}


def _phase_bridge_authorize(_ports: Any, ws: str, context: dict[str, Any], current: Mapping[str, Any]) -> None:
    authority = context["stage"]["authority"]
    lifecycle = context["lifecycle"]
    lifecycle.authority_validator(authority, _ports._current_stage_authority(ws, current, authority))
    indexed = _ports._indexed_stage(context["store"], current, context["run_id"], context["stage"]["stage_id"])
...
def _resolve_phase_retry(_ports: Any, ws: str, state: dict[str, Any], **request: Any) -> dict[str, Any]:
    return resolve_retry(_ports, ws, state, **request)


def _phase_bridge_freshness(_ports: Any, ws: str, scopes: list[str]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    return {"candidate_sha": str(_ports.tp.git_head(ws) or ""),
...
def _phase_bridge_signing(_ports: Any, ws: Any, material: Any, *, admit: Any=False, authorize: Any=None) -> Any:
    from taskplane import review_evidence
    ...
    original = None
    build_collection = material["bindings"]["phase_id"] == "build" and "original_preparation" in material
    if "original_preparation" in material:
        ...
        if stable(material) != stable(original) or recorded_fingerprint != material["freshness"]["impact_manifest_fingerprint"]:
            raise ValueError("current validation changed original preparation")
        if any(freshness[key] != material["freshness"][key] for key in ("candidate_sha", "source_tree")):
            raise ValueError("current validation source changed")
        if build_collection:
            manifest = _ports._stage_store(ws, material["bindings"]["run_id"]).load(material["bindings"]["run_id"])
            if not _ports.stage_loop.authorized_run_revision(_ports, ws, manifest,
                    original["freshness"]["candidate_sha"], freshness["candidate_sha"]):
                raise ValueError("Build output changed outside its approved scope or ancestry")
        elif any(freshness[key] != original["freshness"][key] for key in ("candidate_sha", "source_tree")):
    ...
    if original is None and ({**freshness, "impact_manifest_fingerprint": recorded_fingerprint} != material["freshness"] or
            current_impact.get("policy") != recorded_impact.get("policy")):
        raise ValueError("runtime signing current freshness changed")
    run_id = material["bindings"]["run_id"]
    policy = _ports.phase_harness.resource_policy(_ports._stage_store(ws, run_id).load(run_id), run_id)
    ...
    return _ports.design_host_transport.runtime_receipt_authority(_ports.tp, ws,
        bindings=material["bindings"], freshness=material["freshness"], now=int(_ports.SystemClock().wall_time()),
        admit=admit, authorize=authorize,
        collection_policy=(review_evidence.content_fingerprint({"kind": "approved-build-output",
            "preparation": material["original_preparation"], "freshness": material["freshness"],
            "scope": material["signing_scope"]}) if build_collection else
            None if policy is None else policy["fingerprint"] if original is None else
            review_evidence.content_fingerprint({"policy": policy["fingerprint"], "validation": material["freshness"],
                "preparation": material["original_preparation"]})),
        original_freshness=None if original is None else original["freshness"])


def _phase_bridge_telemetry(_ports: Any, ws: str, completion: Mapping[str, Any], *, terminal_snapshot: Any=None) -> Any:
    ...
    artifacts = review_evidence.ArtifactStore(ws)
    material = artifacts.read((completion.get("validation") or {}).get("signing_material", completion["preparation"]))
    ...
    if start["claim"] != result["start_identity"] or terminal["claim"] != result["terminal_identity"] or \
            terminal["outcome"] not in {"success", "complete"}:
    ...
    inputs = dispatch_telemetry.AttemptTelemetryInputs(ledger=ledger,
        ...
        now=math.ceil(_ports.SystemClock().wall_time()), event_deliveries=tuple(binding["events"]),
        resource_limits_advisory=_ports.phase_harness.resource_policy(
            _ports._stage_store(ws, result["run_id"]).load(result["run_id"]), result["run_id"]) is not None)
...
def _phase_bridge_retro_inputs(_ports: Any, ws: Any, context: Any, domain: Any=None) -> Any:
    ...
    if domain is None:
        ...
        inputs, telemetry_ref = _ports._phase_bridge_telemetry(ws, completion, terminal_snapshot=snapshot)
        ...
        inputs, telemetry_ref = _ports._phase_bridge_telemetry(ws, completion,
...
def _phase_bridge_runtime(_ports: Any, ws: str, context: dict[str, Any], material: Mapping[str, Any]) -> Any:
    ...
    package = tuple(_ports.agent_runtime.Artifact(row["artifact_class"], row["artifact_schema_version"], row["reference"])
        for row in material["package"])
    dispatch = _ports.agent_runtime.Dispatch(material["bindings"], package, knowledge,
    ...
    runtime = _ports.agent_runtime.AgentRuntime(context["registry"], context["artifacts"], nonce,
...
def _phase_bridge_build_owner(_ports: Any, ws: Any, context: Any, runtime: Any, material: Any) -> Any:
    ...
    owner = _ports.loop_recovery.LeaseRecovery(ws, mutate_state=_ports.mutate, clock=runtime.clock,
        authorize=authorized, usage=lambda: {key: runtime.usage()[key] for key in limits},
        limits=limits, stage_id=context["stage"]["stage_id"])
    return owner, lease
...
def _phase_bridge_output_location(_ports: Any, ws: Any, path: Any) -> Any:
    ...
    if (not isinstance(path, str) or not path or _ports.os.path.isabs(path) or
            re.match(r"^[A-Za-z]:", path) or ".." in path.replace("\\", "/").split("/")):
        raise ValueError("phase output path must be workspace-relative")
    return ws, path


def _phase_bridge_preparation_operation(_ports: Any, ws: Any, context: Any, source: Any, binding: Any) -> Any:
    ...
    if any(row["operation"] == "phase_prepare" and
            row["result"].get("stage_fingerprint") == context["stage"]["fingerprint"]
            for row in phase_records.phase_records(current).values()) or \
            ((_ports.load(ws) or {}).get("attempt_leases") or {}).get(context["stage"]["stage_id"]) or _ports.tp._active_worker_contracts(ws):
    ...
    return binding["operation_id"] + "-" + _ports.hashlib.sha256(binding["attempt_id"].encode()).hexdigest()[:32]
...
def _phase_predecessor(_ports: Any, context: Any) -> Any:
    ...
    parent = _ports._indexed_stage(context["store"], context["manifest"], context["run_id"], parents[0])
    if parent["stage_kind"] != "build" or parent["outcome"] != "closed" or parent["authority"] != stage["authority"]:
...
def input_package(runtime: Any, context: Any) -> Any:
    ...
    if predecessor is None:
        return None
    authority = context["stage"]["authority"]
    return runtime.consume_phase_handoff(context["artifacts"], predecessor,
        registry=context["registry"], phase_id=context["stage"]["stage_kind"],
...
def _phase_bridge_prepare(_ports: Any, ws: str, state: Mapping[str, Any], contract: dict[str, Any],
    ...
    binding = {"run_id": context["run_id"], "phase_id": stage["stage_kind"],
        ...
        "sealed_package_fingerprint": _ports.agent_runtime.package_fingerprint(package, knowledge, envelope),
    ...
    prior_lease = (((_ports.load(ws) or {}).get("attempt_leases") or {}).get(stage["stage_id"]) or {}).get("lease") or {}
    ...
    if _ports.phase_harness.resource_policy(context["manifest"], context["run_id"]) is None:
        _ports._phase_bridge_signing(ws, material, admit=True,
            authorize=lambda: _ports._phase_bridge_authorize(ws, context, context["store"].load(context["run_id"])))
    ...
    if stage["stage_kind"] == "build":
        ...
        if stage["stage_kind"] == "engineering":
            kernel = (current_state.get("review_kernel_runs") or {}).get(
                _ports._review_kernel_binding_key("em", _ports._current_task(current_state))) or {}
            if kernel.get("stage") != "review" or _ports.os.path.realpath(str(kernel.get("workspace") or "")) != _ports.os.path.realpath(ws):
                raise ValueError("Engineering requires its own current ReviewKernel selection")
            engineering_source = _ports.review.engineering_phase_source(ws,
        ...
        selection = _ports.review.precommit_evaluator_selection(runtime, [dispatch], binding=selection_binding)
    ...
    worker_input: dict[str, Any] = {"phase_runtime": {"outputs": paths, "package": material["package"]}}
    if stage["stage_kind"] == "design":
        baseline = _ports.depgraph.validate_design_decomposition_receipt(state.get("design_decomposition_receipt"))
    ...
    if stage["stage_kind"] == "engineering":
        ...
        retained = _ports._current_phase_contribution_package(ws, task_state, require_contributions=False)
        if retained is None:
    ...
    if stage["stage_kind"] in {"build", "evaluate"}:
        ...
        worker_input["task"] = {key: task[key] for key in (
            "id", "scope", "tests", "deps", "criteria", "contracts", "design_edges", "new_modules",
            "impact_policy", "failure_routing", "failure_classification") if key in task}
        worker_input["task"]["operation"] = "fix" if task_state.get("step") == "fix" else stage["stage_kind"]
    _ports.phase_harness.compile_brief(context, worker_input,
        _ports.reqs.get_requirement(ws, str(state["requirement_id"])) if definition["entry"] else
        next(context["artifacts"].read(item.reference) for item in package if item.artifact_class == "requirement"), ws)
    if "graph_baseline" in worker_input:
    ...
    current = context["store"].load(context["run_id"])
    receipt = _ports.phase_records.commit_phase_record(context["store"], context["run_id"], expected_revision=current["revision"],
...
def observe_phase_runtime_hook(_ports: Any, ws: str, contract: Mapping[str, Any], event: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    observed = _ports.design_host_transport.observe_phase_hook(_ports.tp, ws, contract, event,
        nonce=source, bindings=dispatch.nonce_bindings)
...
def _collect_phase_attempt(_ports: Any, ws: Any, attempt: Any, *, completed_worker: Any=None) -> Any:
    ...
    if prior is not None:
        ...
        signing_material = artifacts.read((prior["result"].get("validation") or {}).get("signing_material", requested["reference"]))
    ...
    lens_plan = None
    if any(row["artifact_class"] == "lens-evidence" for row in definition["produces"]):
        lens_plan = prepare_lenses(_ports, {**context, "definition": definition}, material,
            artifacts.read(material["worker_input_reference"]), authored)
    package = None
    if stage["stage_kind"] in {"plan", "build"}:
        package = _ports.consume_phase_handoff(artifacts, material["predecessor"], registry=registry,
    ...
    if stage["stage_kind"] in {"evaluate", "engineering"}:
        ...
        if judgment.get("task") != selection["task_id"] or judgment.get("requirement") != selection["requirement_id"]:
            raise ValueError("review judgment belongs to another task or requirement")
        if stage["stage_kind"] == "evaluate":
            _ports.evaluation_output.validate_evaluator_value(judgment, expected_lenses=[],
    ...
    if stage["stage_kind"] != "plan":
        authored = _ports.produce_spec_phase_candidates(artifacts, definition, authored,
            package=package, state=_ports.load(ws), workspace=ws)
    if lens_plan is not None:
        authored["lens-evidence"] = lens_evidence(artifacts, {**material, "lens_plan": lens_plan})
    output_rows = [artifact.projection() for artifact in _ports.store_spec_phase_outputs(artifacts, definition, authored)]
    observation = _ports.agent_runtime.Observation(start["claim"], (), terminal["claim"], "reconciled",
        tuple(_ports.agent_runtime.Artifact(row["artifact_class"], row["artifact_schema_version"], row["reference"])
    ...
    if stage["stage_kind"] == "build":
        ...
        result = _ports.review.complete_evaluator_phase(runtime, dispatch, observation,
            selection_ref=material["domain"]["selection"])
    elif stage["stage_kind"] == "retro":
        _, telemetry = _ports._phase_bridge_retro_inputs(ws, context, material["domain"])
        retro_receipt = _ports.retro_engine.complete_retro_phase(runtime, dispatch, observation, **telemetry)
        result = _ports.retro_engine.read_retro_phase(artifacts, retro_receipt, **telemetry)["runtime_result"]
    else:
    ...
    signed = _ports._phase_bridge_signing(ws, signing_material, admit=runtime.resource_limits_advisory or stage["stage_kind"] == "build",
        authorize=lambda: _ports._phase_bridge_authorize(ws, context, store.load(run_id))).sign(result, store=artifacts)
    authority = stage["authority"]
    handoff = _ports.produce_phase_handoff(artifacts, registry=registry, phase_result=result, dispatch=dispatch,
    ...
    complete = {"runtime_result": artifacts.put("phase-result", result), "handoff": handoff,
        ...
        "timing": {"stage_id": stage["stage_id"], "started_at": start["observed_at"],
                   "completed_at": terminal["observed_at"]}}
    complete["validation"] = {"mode":"current-semantic", "validated_at":_ports.time.time(),
    ...
    complete["resource_usage"] = {**_ports.phase_harness.usage_evidence(ws, material, terminal),
    ...
    def authorize_collection(fresh: Any) -> None:
        ...
        _ports._phase_bridge_signing(ws, signing_material).verify(signed, store=artifacts)
    committed = _ports.phase_records.commit_phase_record(store, run_id, expected_revision=current["revision"],
...
def _phase_bridge_gate_check(_ports: Any, ws: str, state: Mapping[str, Any]) -> None:
    ...
    material = artifacts.read((completion.get("validation") or {}).get("signing_material", completion["preparation"]))
    signed = artifacts.read(completion["runtime_receipt"])
    if _ports._phase_bridge_signing(ws, material).verify(signed, store=artifacts)["payload"] != result:
        raise ValueError("signed runtime differs from accepted output")
    source = _ports.design_host_transport.phase_nonce_source(_ports.tp, ws, result["run_id"], existing_only=True)
    ...
    if (completion.get("validation") or {}).get("completion_source") == "codex-task-complete":
        from taskplane import codex_identity
        start = source._read_phase_hook(source.recover(material["nonce_bindings"]), material["nonce_bindings"], "start")
...
def _phase_bridge_retro_completion(_ports: Any, ws: Any, state: Any) -> Any:
    ...
    if context is None and pending is None:
        return None
    completion = ((pending or {}).get("phase_runtime") or {}).get("completion")
    if context is None or context["stage"]["stage_kind"] != "retro" or not isinstance(completion, _ports.Mapping) or \
    ...
    accepted = _ports.retro_engine.read_retro_phase(context["artifacts"], completion["retro_receipt"], **kwargs)
...
def validate_spec_phase_artifact(_ports: Any, value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    artifact_class = next((name for name, version in _ports.stage_artifacts.SCHEMAS.items() if version == schema), None)
    ...
    if schema == 'taskplane.lens-evidence/v1':
        return copy.deepcopy(dict(value))
    if schema in {'taskplane.source-touchpoint-coverage/v1', 'taskplane.dependency-decomposition/v1', 'taskplane.cross-task-seam-manifest/v1', 'taskplane.realized-seam-conformance/v1'}:
        ...
        if value.get('fingerprint') != review_evidence.content_fingerprint({key: item for key, item in value.items() if key != 'fingerprint'}):
...
def store_spec_phase_outputs(_ports: Any, store: Any, definition: Mapping[str, Any], authored: Mapping[str, Mapping[str, Any]]) -> tuple[Any, ...]:

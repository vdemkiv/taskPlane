...
| **v2.17.4** | **PR reviews now treat available requirement artifacts as first-class evidence and validate them criterion by criterion.** Commit-body implementation claims become acceptance criteria, structured review instructions request matching lenses through the live catalog, and ambiguous behavioral claims remain unverified until evidence can prove them. The inline dashboard now has distinct DoR and requirements-validation sections with `met`, `partial`, `not met`, and `cannot verify` verdicts, related findings, validation mode, and gate impact. Its controls use CSP-safe delegated actions and a self-contained light/dark theme, while dynamic-only review no longer fabricates a human rendering decline. Validation repairs can run in a disposable push-disabled PR copy without modifying the reviewed checkout. |

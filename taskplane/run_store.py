...
def _stage_entities_module():
    ...
    try:
        module = (
            importlib.import_module(".stage_entities", package=__package__)
            if __package__
            else importlib.import_module("stage_entities")
        )
...
_STAGE_SUMMARY_FIELDS = frozenset(
    {
        ...
        "completion_evidence_fingerprints",
        "actor",
        "terminalized_at",
        "reason_code",
        "reason",
...
def _confined_directory_fd(home: str, directory: str):
    ...
    if (
        ...
        raise StageStateError("stage object storage needs no-follow dir-fd support")
    ...
    try:
        ...
        for component in components:
            if component in {"", ".", ".."}:
                raise StageStateError("stage object directory component is invalid")
        ...
        raise StageStateError("stage object directory is unavailable") from exc
    finally:
        os.close(descriptor)
...
def _active_stage_ids(stage_heads: object) -> list[str]:
    ...
    for raw_stage_id, head in stage_heads.items():
        ...
        if not isinstance(reference, dict) or set(reference) != {
            ...
            raise StageStateError(f"stage head {stage_id} object reference is invalid")
        if (
            reference.get("schema") != "taskplane.stage-object-ref/v1"
            or reference.get("stage_id") != stage_id
        ):
            raise StageStateError(f"stage head {stage_id} object reference identity mismatch")
        object_fingerprint = _fingerprint(reference.get("fingerprint"), "stage object fingerprint")
        ...
        if reference.get("locator") != (f"stages/objects/{stage_id}/{object_fingerprint}.json"):
            raise StageStateError("stage object locator mismatch")
        summary = head.get("summary")
        if not isinstance(summary, dict) or set(summary) != _STAGE_SUMMARY_FIELDS:
            raise StageStateError(f"stage head {stage_id} needs a bounded summary")
        if len(_canonical_json_bytes(summary)) > _MAX_STAGE_SUMMARY_BYTES:
            raise StageStateError(f"stage head {stage_id} summary exceeds its byte bound")
        if (
            summary.get("schema") != "taskplane.stage-summary/v1"
            or summary.get("stage_id") != stage_id
        ):
            raise StageStateError(f"stage head {stage_id} summary identity mismatch")
        aggregate_fingerprint = _fingerprint(
            summary.get("aggregate_fingerprint"), "stage aggregate fingerprint"
        )
        if (
            summary.get("stage_fingerprint") != aggregate_fingerprint
            or object_fingerprint != aggregate_fingerprint
        ):
            raise StageStateError(f"stage head {stage_id} fingerprint binding mismatch")
        ...
        if summary.get("fingerprint") != expected_summary_fingerprint:
            raise StageStateError(f"stage head {stage_id} summary fingerprint mismatch")
        ...
        if state == "active":
            ...
            if summary.get("outcome") not in {"done", "closed", "discarded"}:
                raise StageStateError("terminal stage summary outcome is invalid")
        else:
            raise StageStateError(f"stage head {stage_id} has invalid lifecycle state")
    return sorted(active)


def _canonical_active_projection(stage_heads: object, foreground_stage_id: object = None) -> dict:
    active = _active_stage_ids(stage_heads)
    foreground = (
        None
        if foreground_stage_id is None
        else _nonempty_text(foreground_stage_id, "foreground stage id")
    )
    ...
    projection["fingerprint"] = hashlib.sha256(_canonical_json_bytes(projection)).hexdigest()
    return projection


def _validate_active_projection(stage_heads: object, projection: object) -> dict:
    if not isinstance(projection, dict):
        raise StageStateError("active_stage_projection must be an object")
    if set(projection) != {"schema", "active_stage_ids", "foreground_stage_id", "fingerprint"}:
    ...
    expected = _canonical_active_projection(stage_heads, projection.get("foreground_stage_id"))
...
def _validate_lineage_bindings(lineage: list[dict], stage_heads: dict) -> None:
    ...
    for row in lineage:
        ...
        if child_id not in stage_heads or any(
            predecessor_id not in stage_heads for predecessor_id in predecessor_ids
        ):
            raise StageStateError("stage lineage names an unindexed stage")
        child_summary = stage_heads[child_id]["summary"]
        if parent_id is not None and (
            parent_id not in stage_heads or parent_id not in child_summary["parent_stage_ids"]
        ):
            raise StageStateError("stage lineage parent does not match the child head")
        if (
            row["handoff_fingerprint"] != child_summary["input_manifest_fingerprint"]
            or predecessor_ids != child_summary["predecessor_stage_ids"]
        ):
            raise StageStateError("stage lineage does not match the child head")
...
def _validate_stage_journal_outbox(value: object, *, operations: dict) -> None:
    if value is None:
        return
    ...
    for operation_id, entry in value.items():
        ...
        event = entry["event"]
        if not isinstance(event, dict) or set(event) != _STAGE_JOURNAL_EVENT_FIELDS:
...
def _validate_stage_index(manifest: dict) -> None:
    ...
    for operation_id, receipt in operations.items():
        _validate_operation_receipt(
            receipt,
            operation_id=_operation_id(operation_id, "stage operation id"),
            manifest_revision=revision,
        )
    _validate_stage_journal_outbox(manifest.get("stage_journal_outbox"), operations=operations)
...
def _operation_receipt(
    value: object, *, operation_id: str, request_fingerprint: str, committed_revision: int
) -> dict:
    if not isinstance(value, dict):
    ...
    if not isinstance(raw_stage_ids, list) or any(
        not isinstance(value, str) or not value for value in raw_stage_ids
    ):
    ...
    if "result" in receipt:
        ...
        if supplied_result not in (None, expected_result):
            raise StageStateError("stage operation result fingerprint mismatch")
        receipt["result_fingerprint"] = expected_result
    elif "result_fingerprint" in receipt:
        raise StageStateError("stage operation result fingerprint has no result")
    _validate_operation_receipt(
        receipt, operation_id=operation_id, manifest_revision=committed_revision
    )
    return receipt


def _validate_operation_receipt(
    value: object, *, operation_id: str, manifest_revision: int
) -> dict:
    if not isinstance(value, dict):
    ...
    if value.get("schema") != _STAGE_RECEIPT_SCHEMA or value.get("operation_id") != operation_id:
        raise StageStateError("stored stage operation receipt identity mismatch")
    _fingerprint(value.get("request_fingerprint"), "stage operation request fingerprint")
    ...
    try:
        ...
        raise StageStateError("stored stage operation stage ids are invalid") from exc
    if stage_ids != sorted(set(stage_ids)):
        raise StageStateError("stored stage operation stage ids must be sorted and unique")
    if not stage_ids and value.get("operation") not in _EMPTY_STAGE_ID_OPERATIONS:
    ...
    if (
        ...
        or revision < 1
        or revision > int(manifest_revision)
    ):
        raise StageStateError("stored stage operation committed revision is invalid")
    if "result" in value:
        ...
        if value.get("result_fingerprint") != expected_result:
            raise StageStateError("stored stage operation result fingerprint mismatch")
...
def _lock(path: str):
    ...
    try:
        ...
        raise RunStoreBusy(f"run manifest lock is unavailable: {exc}") from None
...
class RunStore:
    ...
    def apply_knowledge(
        ...
        *,
        ...
        writer_id: str,
        fencing_token: int,
        current_writer: Callable[[], tuple[str, int]],
        ...
        request = hashlib.sha256(
            _canonical_json_bytes(
                {
                    "proposal": value["proposal_fingerprint"],
        ...
        with _lock(path):
            ...
            fence_valid = callable(current_writer) and current_writer() == (
                writer_id,
                fencing_token,
            )
            ...
            if not gate_valid or not fence_valid or not retention_valid:
                ...
                action == "apply" and value["base_knowledge_fingerprint"] != snapshot["fingerprint"]
            ):
            ...
            if outcome in {"applied", "tombstoned"}:
                entry = {
                    ...
                    "observation_fingerprint": hashlib.sha256(
                        str(value["finding_or_observation_reference"]).encode()
                    ).hexdigest(),
                ...
                new_fingerprint = hashlib.sha256(
                    _canonical_json_bytes(
                        {
                            "scope": scope,
                            "lineage": next_snapshot["lineage"],
                            "content": next_snapshot["content"],
                        }
                    )
                ).hexdigest()
            ...
            result = entities.create_contract(
                {
                    ...
                    "applied_at": datetime.fromtimestamp(now, timezone.utc)
                    .isoformat()
                    .replace("+00:00", "Z"),
                    "outcome": outcome,
                    "continuation": {"kind": reason, "phase_id": value["phase_id"]},
                }
            )
            receipt = stage_handoff.sign_contract(
                result, key=key, issued_at=now, expires_at=expires_at, freshness=freshness
            )
            if outcome in {"applied", "tombstoned"}:
    ...
    def _read_journal_rows_locked(self, run_id: str) -> list[dict]:
        ...
        for index, encoded in enumerate(lines):
            final_unterminated = index == len(lines) - 1 and not encoded.endswith((b"\n", b"\r"))
            try:
    ...
    def _journal_contains_stage_event(
        self, run_id: str, event: dict, rows: list[dict] | None = None
    ) -> bool:
        ...
        matches = [
            ...
            for row in rows
            if (
                row.get("event") == "stage_operation_committed"
                and (row.get("operation_id"), row.get("request_fingerprint")) == identity
            )
        ]
    ...
    def _relay_stage_journal_outbox_locked(
        self, run_id: str, manifest: dict, operation_id: str, journal_rows: list[dict] | None = None
    ) -> dict:
    ...
    def _relay_all_stage_journal_outbox_locked(self, run_id: str, manifest: dict) -> dict:
        ...
        for operation_id in sorted((manifest.get("stage_journal_outbox") or {}).keys()):
    ...
    def _load_manifest(self, run_id: str) -> dict:
        ...
        try:
            ...
            raise RunStoreError(f"run manifest is unavailable: {run_id}") from exc
        if not isinstance(value, dict):
        ...
        if "run_artifacts" in value:
            try:
                run_artifacts.validate_manifest_locator_reference(value["run_artifacts"])
            except run_artifacts.RunArtifactError as exc:
                raise RunStoreError(f"run artifact locator is invalid: {run_id}") from exc
        return value
    ...
    def create(
        ...
        *,
        ...
        layout = storage.resolve_layout(identity, home=self.home, run_id=run_id)
        path = self._manifest_path(run_id)
        ...
        with _lock(path):
            ...
            manifest = {
                ...
                "preflight": {"status": "pending", "completed_steps": [], "pending_action": None},
    ...
    def load(self, run_id: str) -> dict:
        ...
        with _lock(path):
            value = self._load_manifest(run_id)
            return self._relay_all_stage_journal_outbox_locked(run_id, value)
    ...
    def save_workflow(self, run_id: str, workflow: dict) -> dict:
        ...
        with self.transaction(run_id):
            ...
            if current.get("workflow") == workflow:
                return current
            updated = self.commit(
                run_id, expected_revision=current["revision"], changes={"workflow": None}
            )
            updated["workflow"] = copy.deepcopy(workflow)
            _atomic_write_json(self._manifest_path(run_id), updated)
            return updated

    def commit(self, run_id: str, *, expected_revision: int, changes: dict) -> dict:
        if not isinstance(changes, dict):
            raise RunStoreError("run changes must be an object")
        forbidden = set(changes) & (_RUN_OWNED_FIELDS | _STAGE_INDEX_KEYS)
        if forbidden:
            raise RunStoreError(
                "generic run commit cannot change owned fields: " + ", ".join(sorted(forbidden))
            )
        path = self._manifest_path(run_id)
        with _lock(path):
            current = self._load_manifest(run_id)
            current = self._relay_all_stage_journal_outbox_locked(run_id, current)
            actual = int(current.get("revision") or 0)
            if actual != int(expected_revision):
                raise RevisionConflict(
                    f"run {run_id} revision is {actual}, expected {expected_revision}"
                )
            _validate_stage_index(current)
    ...
    def commit_stage_operation(
        ...
        *,
        ...
        request_fingerprint = _fingerprint(request_fingerprint, "request fingerprint")
        ...
        with _lock(path):
            ...
            if previous is not None:
                ...
                checked_previous = _validate_operation_receipt(
                    previous, operation_id=operation_id, manifest_revision=actual
                )
                if checked_previous.get("request_fingerprint") != request_fingerprint:
                    raise OperationConflict(
                        f"operation id {operation_id} was reused with "
                        "a different request fingerprint"
                    )
                checked = _current_stage_index(current)
                self._relay_stage_journal_outbox_locked(run_id, checked, operation_id)
            ...
            if validate_authority is None:
                raise StageStateError("stage operation requires authority revalidation")

            checked = _current_stage_index(current)
            if validate_authority is not None:
                validate_authority(copy.deepcopy(checked))
            result = mutate(copy.deepcopy(checked))
            if not isinstance(result, dict) or set(result) != {"changes", "receipt"}:
                raise StageStateError("stage mutate must return changes and receipt")
            ...
            if invalid_changes:
                raise StageStateError(
                    "stage operation cannot change fields: " + ", ".join(sorted(invalid_changes))
                )
            missing_changes = _STAGE_MUTATION_KEYS - set(changes)
            if missing_changes:
                raise StageStateError(
                    "stage operation must replace fields: " + ", ".join(sorted(missing_changes))
                )

            updated = copy.deepcopy(checked)
            for key, value in changes.items():
                if key in {"stage_heads", "lineage", "active_stage_projection"}:
            ...
            changed_heads = sorted(
                stage_id for stage_id, head in new_heads.items() if old_heads.get(stage_id) != head
            )
            stage_entities = _stage_entities_module()
            for changed_stage_id in changed_heads:
                changed_head = new_heads[changed_stage_id]
                indexed_stage = self.read_stage_object(run_id, changed_head["object"])
                expected_summary = stage_entities.bounded_stage_summary(indexed_stage)
                if expected_summary != changed_head["summary"]:
                    raise StageStateError("stage head summary does not match immutable object")
            if receipt["operation"] == "resume_stage":
                if changed_heads or len(receipt["stage_ids"]) != 1:
                    raise StageStateError("resume receipt must bind one unchanged stage head")
                resumed_id = receipt["stage_ids"][0]
                resumed = new_heads.get(resumed_id)
                if (
                    resumed is None
                    or resumed["summary"]["state"] != "active"
                    or new_lineage != old_lineage
                    or updated["active_stage_projection"] != checked["active_stage_projection"]
                ):
                    raise StageStateError("resume may only claim an active stage attempt")
            elif receipt["stage_ids"] != changed_heads:
                raise StageStateError("stage operation receipt does not bind changed heads")
            for stage_id in set(old_heads) & set(changed_heads):
                ...
                if new_summary["aggregate_revision"] != old_summary["aggregate_revision"] + 1:
                    raise StageStateError("stage aggregate revision must advance exactly once")
            updated["stage_operations"] = copy.deepcopy(checked.get("stage_operations") or {})
            ...
            _validate_stage_index(updated)
            _atomic_write_json(path, updated)
            self._relay_stage_journal_outbox_locked(run_id, updated, operation_id)
    ...
    def rebuild_active_stage_projection(
        ...
        *,
        ...
        with _lock(path):
            ...
            expected = _canonical_active_projection(heads, foreground_stage_id)
            ...
            repair_id = (
                _operation_id(operation_id, "operation id")
                if operation_id is not None
                else f"projection-repair:{int(expected_revision)}"
            )
            request = hashlib.sha256(
                _canonical_json_bytes(
                    {
            ...
            if previous is not None:
                ...
                if checked.get("active_stage_projection") != expected:
                    raise StageStateError("committed projection repair result is not present")
                relayed = self._relay_all_stage_journal_outbox_locked(run_id, checked)
            ...
            if current.get("active_stage_projection") == expected:
                _validate_stage_index(current)
                relayed = self._relay_all_stage_journal_outbox_locked(run_id, current)
                return copy.deepcopy(relayed)
            ...
            _validate_stage_index(updated)
            _atomic_write_json(path, updated)
            relayed = self._relay_stage_journal_outbox_locked(run_id, updated, repair_id)
            return copy.deepcopy(relayed)
    ...
    def put_stage_object(self, run_id: str, stage: dict) -> dict:
        ...
        path = storage.stage_object_path_for_run(self.home, run_id, stage_id, fingerprint)
        ...
        with _confined_directory_fd(self.home, parent) as parent_fd:
            try:
                        ...
                        try:
                            ...
                            with os.fdopen(read_descriptor, "rb") as handle:
                                if not stat.S_ISREG(os.fstat(handle.fileno()).st_mode):
                                    raise StageStateError("stage object path is not a regular file")
                                existing = handle.read(len(payload) + 1)
                        except OSError as exc:
                            raise StageStateError("stage object is unavailable") from exc
                        if existing != payload:
                            raise StageStateError("stage object path contains different bytes")
                    os.fsync(parent_fd)
                finally:
                    try:
                ...
                raise StageStateError("stage object could not be stored") from exc
        # Confirm that the pinned directory is still the canonical locator.
        checked_path = storage.stage_object_path_for_run(self.home, run_id, stage_id, fingerprint)
    ...
    def read_stage_object(self, run_id: str, reference: dict) -> dict:
        ...
        fingerprint = _fingerprint(reference.get("fingerprint"), "stage fingerprint")
        ...
        if (
            ...
            or byte_count <= 0
            or byte_count > _MAX_STAGE_RECEIPT_BYTES
        ):
            raise StageStateError("stage object byte count is invalid")
        expected_locator = f"stages/objects/{stage_id}/{fingerprint}.json"
        ...
        with _confined_directory_fd(self.home, parent) as parent_fd:
            try:
                with os.fdopen(
                    os.open(leaf, os.O_RDONLY | os.O_NOFOLLOW, dir_fd=parent_fd), "rb"
                ) as handle:
        ...
        if (
            checked.get("run_id") != run_id
            or checked.get("stage_id") != stage_id
            or checked.get("fingerprint") != fingerprint
        ):
            raise StageStateError("stage object identity mismatch")
        return copy.deepcopy(checked)

    def claim_stage_execution_root(
        self, run_id: str, *, stage_id: str, execution_root_id: str, attempt_id: str | None = None
    ) -> dict:
        ...
        return storage.claim_stage_execution_root_for_run(
            ...
            attempt_id=(_stage_id(attempt_id, "attempt id") if attempt_id is not None else None),
        )

    def register_checkout(
        self, identity: storage.RepositoryIdentity, *, checkout: str, source: str
    ) -> dict:
        """Register a non-authoritative checkout alias without moving it."""
        layout = storage.resolve_layout(identity, home=self.home, run_id="checkout-registration")
        ...
        with _lock(path):
            try:
                ...
                raise RunStoreError(f"repository record is unavailable: {path}") from exc
            if (
                record.get("schema") != "taskplane.repository/v1"
                or (record.get("repository") or {}).get("repo_id") != identity.repo_id
            ):
                raise RunStoreError("repository record identity mismatch")
            rows = list(record.get("checkouts") or [])
            row = {"path": os.path.realpath(checkout), "source": str(source)}
            if row not in rows:
                rows.append(row)
            record["checkouts"] = sorted(
                rows, key=lambda item: (str(item.get("path")), str(item.get("source")))
            )
            _atomic_write_json(path, record)
            return record

    def reference_command(
        self, run_id: str, *, expected_revision: int, handle: str, wave_id: str | None = None
    ) -> dict:

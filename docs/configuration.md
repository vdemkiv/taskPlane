...
| `TASKPLANE_SESSION_ID` | *(unset; falls back to `CODEX_THREAD_ID` or `CLAUDE_CODE_SESSION_ID`; `CLAUDE_SESSION_ID` is a compatibility alias)* | Stable host-session identity required by new v4 runs and bound into the root bootstrap authority. Replay under a different session is refused. | **Yes** — it supplies attributable session identity; it does not grant approval by itself. |

...
def new(ws: str, name: str, goal: str, requirement_id: str | None = None) -> dict:
    """Register planning metadata without replacing the selected run."""
...
def switch(ws: str, name: str) -> dict:
    """Track metadata cannot switch a run's durable execution identity."""
    return {"error": "track switching is retired; use a separate run workspace",
            "code": "unsupported_track_switch"}

...
| Version | Highlights |
| --- | --- |
| **v2.10.0** | **Nine defects a real upstream repo found that no self-review could.** v2.9.0 was run end-to-end against `aws/karpenter-provider-aws#9464` in a separate session. The harness held — not one write reached reviewed source — and nine defects surfaced that only appear at somebody else's scale. The worst was silent: `tp findings` printed `0 high · 3 med · 13 low` while the findings file carried `class: regression`, which the engine's own gate blocks; the reviewer read the headline, reported "0 confirmed regressions", and recommended approve. The headline now reads the blocking set off `loop.classify_findings` — never a second implementation — and says `1 BLOCK (1R·0H·1P·0O)`. Parallel lens dispatch did not work with more than one agent: six sibling lens contracts intersected to the EMPTY set, so 4 of 6 lenses produced no evidence and the wave board read 2/6 for a finished review — sibling waves now merge write-allows and SUM budgets, while contracts over genuinely separate trees still intersect to nothing. The graph could not see intra-repo Go at all: a root `go.mod` was skipped, so every `pkg/**` import landed as `ext:` and `graph impact` reported 2 modules on a 256-module repo. And six lens agents each burned actions rediscovering that `go test` could not run — one fact about the checkout, paid for six times: `lens dispatch` now probes build/test runnability ONCE with a bounded cheap subcommand (never the suite), states it in every brief with a do-not-re-probe instruction, and caches it per tree state so a whole wave shares one answer, and `lens dispatch --resume` re-briefs only the lanes with no findings on disk — four of ten sub-agent transcripts in the measured session existed because an interrupted wave was paid for twice. Plus: a review dashboard that is a standalone document when it leaves the session, `tp version --verify` reading the Codex manifest it actually ships, and `echo tp clear` no longer read as a release command. 1,905 tests. |

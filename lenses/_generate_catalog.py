...
("Engineering craft","backend","Back-end engineering",
 ...
 ["backend","api"],None,[],[]),
("Architecture & systems","tradeoffs","Design trade-offs",
 "every significant design choice names >=2 real alternatives with an explicit trade-off table: gained / given up / revisit-when; the chosen option is recorded as a proposed decision (D-record) in the registry",
 "the final call -> human at the gate; product scope -> product; overall structure -> architecture",
 ...
 ["**/architecture/**","**/adr/**","**/design/**","**/*.arch.md","plan/**","**/specs/**"],
 ["greenfield","system-design","distributed","integration","feature"],None,
 ["**/architecture/**","**/adr/**","**/design/**"],[]),
("Architecture & systems","solution-design","Solution design",
 "coherence and implementability of a proposed HOW: requirement-to-decision-to-modules/contracts-to-validation/rollout traceability",
 "component/data-flow correctness -> architecture; alternative quality -> tradeoffs; interaction/visual experience -> design",
 "unsupported leaps from requirements to components, missing contract ownership, graph boundaries or validation mappings, internally inconsistent failure/rollout choices, designs that cannot be implemented or reviewed as written",
 ["design/**","**/solution-design/**","**/*.design.md"],
 ["solution-design"],None,["design/**","**/solution-design/**","**/*.design.md"],[]),
("Architecture & systems","services-selection","Tool & services selection",
 "build-vs-buy and dependency choices: managed vs self-hosted, vendor lock-in and exit cost, maturity, license, operational load",
 "code-level dependency hygiene -> code-quality; runtime ops -> devops/sre; live pricing/vendor data -> out of scope (reasons from the repo only)",
 ...
 ["**/package.json","**/requirements*.txt","**/pyproject.toml","**/go.mod","**/Cargo.toml","**/docker-compose*","**/*.tf","**/Gemfile","**/pom.xml"],
 ["integration","greenfield","infrastructure"],None,[],[]),
("Product & delivery","time-to-market","Time to market",
 "delivery speed as a first-class criterion: the fastest credible path, what can be deferred (recorded as debt, not lost), reversible-now over perfect-later",
 "quality floors are NOT negotiable -> security/testability baselines still apply; long-term structure -> architecture",
 ...
 ["plan/**","**/specs/**","**/roadmap*","**/*.spec.md"],
 ["feature","greenfield","prototype"],None,[],[]),

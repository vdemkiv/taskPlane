...
def _seal(value: Any) -> None:
    if value["fingerprint"] != content_fingerprint({k: v for k, v in value.items() if k != "fingerprint"}):
        raise ValueError("phase artifact fingerprint is stale")


def requirement(value: Any) -> None:
    _object(value, {"schema": str, "id": str, "acceptance_criteria": list},
        {"title": str, "summary": str, "functional": list, "non_functional": dict,
         "contracts": list, "depends_on": list, "scope": list, "out_of_scope": list}, label="requirement")
    _strings(value["acceptance_criteria"], "acceptance_criteria", nonempty=True)


def design(value: Any) -> None:
    _object(value, {"schema": str, "requirement": str, "acceptance_map": list, "test_strategy": dict},
        {"title": str, "summary": str, "decision": str, "current_state": dict,
         "alternatives": list, "selected_approach": str, "modules": dict,
         "contracts": list, "graph": dict, "risks": list, "failure_modes": list,
         "observability": dict, "rollout": dict, "visualization": dict,
         "design_counts": dict, "journeys": list, "producers": list, "seam_contracts": list,
         "acceptance_pair_map": dict, "requirement_fingerprint": str,
         "fingerprint": str, "quality_authority": dict, "open_questions": list,
         "lens_evidence": list, "test_strategy_reference": dict}, label="design")
    if not value["acceptance_map"]:
        raise ValueError("Design requires acceptance mappings")
    for row in value["acceptance_map"]:
        _object(row, {"criterion": str}, {"criterion_id": str, "design_element": str,
            "validation": str, "tests": list, "selectors": list, "producer": str,
            "consumers": list, "edges": list}, label="acceptance mapping")
    ...
    if strategy.get("schema") == SCHEMAS["test-strategy"]:
        ...
        reference = _object(selected, {"schema": str, "path": str,
            "strategy_fingerprint": str}, label="test strategy reference")
...
def test_strategy(value: Any) -> None:
    ...
    _object(value, {"schema": str, "acceptance_criteria": list, "producers": list,
        "failure_policy": dict, "validation": dict, "contract_fingerprint_sha256": str}, label="test strategy")
    validate_strategy(value)


def plan_task(value: Any) -> None:
    _object(value, {"schema": str}, {"task": dict, "plan": dict,
        "dependency_outputs": dict, "traceability": dict, "owners": dict,
        "acceptance": list}, label="plan task")
...
def source_coverage(value: Any) -> None:
    _object(value, {"schema": str, "producer": str, "source_tree": str,
        "status": str, "complete": bool, "limits": dict, "kinds": list,
        "touchpoints": dict, "stopping_conditions": list, "fingerprint": str}, label="source coverage")
    _seal(value)
    if value["status"] != "complete" or value["complete"] is not True or value["stopping_conditions"]:
        raise ValueError("partial source coverage cannot enter a phase package")


def decomposition(value: Any) -> None:
    _object(value, {"schema": str, "source_tree": str, "coverage_fingerprint": str,
        "components": list, "edges": list, "tasks": list, "fingerprint": str}, label="decomposition")
    for row in value["components"]:
        _object(row, {"id": str, "module": str, "files": list, "symbols": list, "fingerprint": str}, label="component")
...
def seam_manifest(value: Any) -> None:
    _object(value, {"schema": str, "binding": dict, "decomposition_fingerprint": str,
        "nodes": list, "seams": list, "fingerprint": str}, label="seam manifest")
    _strings(value["nodes"], "seam nodes")
    for row in value["seams"]:
        _object(row, {**{key: str for key in ("id", "producer", "consumer", "kind", "schema_version",
            "cardinality", "producer_symbol", "consumer_symbol", "positive", "severed",
            "producer_owner", "consumer_owner")}, "producer_files": list, "consumer_files": list,
            "producer_order": int, "consumer_order": int}, label="seam")
    _seal(value)


def realized_conformance(value: Any) -> None:
    _object(value, {"schema": str, "status": str, "binding": dict, "manifest_fingerprint": str,
        "realized_decomposition_fingerprint": str, "realized_source_tree": str,
        "missing": list, "unexpected": list, "fingerprint": str},
        {"task_scope": dict}, label="realized conformance")
    if "task_scope" in value:
        scope = _object(value["task_scope"], {"task_id": str, "required_tasks": list,
            "required_nodes": list}, label="conformance task scope")
...
def stage(value: Any) -> None:
    _object(value, {**{key: str for key in ("schema", "run_id", "stage_id", "stage_kind",
        "execution_root_id", "state", "created_at", "fingerprint")},
        **{key: list for key in ("parent_stage_ids", "predecessor_stage_ids", "deliverables",
        "selected_artifacts", "dependencies", "contracts")}, "requirement": dict,
        "design": (dict, type(None)), "input_manifest_ref": dict, "budget": dict,
        "authority": dict, "outcome": (str, type(None)), "default_consumable": bool,
        "terminal": (dict, type(None)), "aggregate_revision": int}, label="stage")
    _seal(value)
...
def judgment(value: Any) -> None:
    ...
    if not isinstance(evaluation, dict) or evaluation.get("status") != "complete" or evaluation.get("reason_code") != "none":
    ...
    elif aggregate is not None or (value["verdict"] == "pass" and not isinstance(value.get("child_evidence"), dict)):
...
VALIDATORS = {"lens-evidence": lens_evidence, "requirement": requirement, "design": design, "test-strategy": test_strategy,
    "plan-task": plan_task, "source-coverage": source_coverage, "decomposition": decomposition,
    "seam-manifest": seam_manifest, "realized-conformance": realized_conformance,
    "stage": stage, "judgment": judgment}


def validate(artifact_class: str, value: object) -> dict[str, Any]:
    if artifact_class not in SCHEMAS or not isinstance(value, Mapping) or value.get("schema") != SCHEMAS[artifact_class]:
...
def evaluator_output_schema() -> dict[str, Any]:
    ...
    return {
        ...
        **_schema_object({
            ...
            "accepted_evaluations": {"type": "array", "items": _schema_object({
                "task_id": string, "candidate_sha": string,
                **{key: _schema_object({
                    "schema": {"const": "taskplane.artifact-reference/v1"},
                    "kind": string, "fingerprint": string, "digest": string, "bytes": {"type": "integer"},
                    "locator": string, "transport": {"const": "artifact-reference"}},
                    ["schema", "kind", "fingerprint", "digest", "bytes", "locator", "transport"])
                   for key in ("judgment", "handoff", "runtime_receipt", "lens_evidence")}},
                ["task_id", "candidate_sha", "judgment", "handoff", "runtime_receipt", "lens_evidence"])},
            "failures": {
                "type": "array",

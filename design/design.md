...
The original progressive-review design remains intact: four mandatory deep floors (architecture, code-quality, security, QA), evidence-selected deep review plus at most one light sweep, attributable promotions, immediate immutable provisional request-changes for severe harm, complete exactly-once conservation for approval, metadata-only repair, exact evaluator/root binding and outage caching, documentation-aware routing, measured convergence, and isolated lens telemetry.

...
def _definition():
    return {
        "schema": "taskplane.phase-definition/v1", "id": "build",
        "role": "tp-executor", "skill_ref": "skill:build",
        "skill_content_fingerprint": "a" * 64,
        "consumes": [], "produces": [], "domain_validator_refs": ["validator:build"],
        "validator_inventory_fingerprint": "b" * 64,
        "capability_requirements": [], "working_lenses": [],
        "evaluation_lenses": [], "budget": {"tokens": 100, "wall_ms": 1000,
                                              "attempts": 1, "corrections": 0},
        "model_tier": "verified", "gate": "mechanical",
        "predecessors": [], "successors": [], "edge_conditions": [],
        "entry": True, "terminal": True, "telemetry_scope": ["build"],
    }


def _runtime():
    bindings = {key: "a" * 64 for key in (
        "candidate_fingerprint", "definition_set_fingerprint", "phase_definition_fingerprint",
        "skill_content_fingerprint", "validator_inventory_fingerprint",
        "capability_set_fingerprint", "sealed_package_fingerprint", "knowledge_fingerprint",
        "authority_fingerprint", "nonce_digest")}
    return {
        "schema": "taskplane.agent-runtime/v1", **bindings,
        "run_id": "run-1", "phase_id": "build", "attempt_id": "attempt-1",
        "operation_id": "operation-1", "validator_identities": ["validator:build"],
        "consumed_artifact_schema_versions": [], "produced_artifact_schema_versions": [],
        "host_kind_version": "codex:test", "lease_id": "lease-1", "fencing_token": 1,
        "deadline": "2026-09-06T12:00:00Z", "budget": _definition()["budget"],
        "start_identity": "start-1", "progress_identity": [], "terminal_identity": "terminal-1",
        "effect_state": "effect_free", "collected_output_references": [],
        "produces_conformance": True, "knowledge_proposals": [],
        "evaluator_dispatch_eligibility": True, "retry_class": "none",
...
def _proposal():
    return {
        "schema": "taskplane.knowledge-update/v1", "base_knowledge_fingerprint": "a" * 64,
        "run_id": "run-1", "phase_id": "build", "attempt_id": "attempt-1",
        "operation_id": "operation-1", "candidate_fingerprint": "b" * 64,
        "finding_or_observation_reference": "artifact://finding/" + "c" * 64,
        "scope": ["taskplane"], "content_class": "fact", "content": "A bounded fact.",
        "supersedes": [],
    }
...
def _apply_receipt():
    return {
        ...
        **{key: "a" * 64 for key in ("base_fingerprint", "current_fingerprint", "new_fingerprint",
                                     "proposal_fingerprint", "gate_receipt_fingerprint")},
        "operation_id": "operation-1", "writer_id": "knowledge-owner", "fencing_token": 1,
        "retention_class": "minimized", "applied_at": "2026-09-06T10:00:00Z",
        "outcome": "applied", "continuation": {"kind": "continue", "phase_id": "build"},
    }
...
def _produce(kind, store):
    ...
    if kind == "handoff-v2":
        ...
        result.update(schema="taskplane.stage-handoff/v2", phase_result=entities.create_contract(_runtime()),
                      produced_artifacts=[{"artifact_class": "delivery", "artifact_schema_version": "v1",
                                           "reference": original["selected_artifacts"][0]}],
                      inherited_artifacts=[], knowledge_apply_receipts=[],
                      unresolved_issues=[])
        return entities.create_contract(result, store=store)
    return entities.create_contract({"definition": _definition, "runtime": _runtime,
                                     "proposal": _proposal, "apply": _apply_receipt}[kind]())
...
def _signed(value, store, key=None):
    return handoff.sign_contract(value, key=key or _key(), issued_at=20, expires_at=60,
                                 freshness=FRESHNESS, store=store)


def _consume(value, signed, store, **kwargs):
    return handoff.verify_contract(signed, trusted_keys={"key-1": _key()},
        expected_schema=value["schema"], expected_freshness=FRESHNESS, now=30, store=store, **kwargs)


@pytest.mark.parametrize("kind", ["stage", "handoff-v1", "handoff-v2", "definition", "runtime", "proposal", "apply"])
def test_contract_matrix_mixed_versions(tmp_path, kind):
    ...
    assert entities.canonical_contract_bytes(dict(reversed(list(value.items()))), store=store) == original
    if kind == "handoff-v1":
...
@pytest.mark.parametrize("case", ["active", "rotation", "revoked", "compromised", "expired",
    "unknown_key", "wrong_key", "not_yet_valid", "invalid_algorithm", "severed_entity_output",
    "candidate_sha", "source_tree", "impact_manifest_fingerprint", "canonical_unicode"])
def test_signature_lifecycle(tmp_path, case):
    ...
    reason = None
    if case == "rotation":
        keys = handoff.rotate_signing_key(keys, "key-1",
            handoff.SigningKey("key-2", b"n" * 32, not_before=25, not_after=120), at=25)
        newer = handoff.sign_contract(value, key=keys["key-2"], issued_at=30, expires_at=60,
                                     freshness=FRESHNESS, store=store)
        assert handoff.verify_contract(newer, trusted_keys=keys, expected_schema=value["schema"],
            expected_freshness=FRESHNESS, now=30, store=store)["authority_valid"]
        ...
        keys, reason = {"key-1": handoff.SigningKey("key-1", b"x" * 32, not_before=10, not_after=100)}, "signature"
        ...
        signed = _signed(value, store)
    args = dict(trusted_keys=keys, expected_schema=value["schema"],
                expected_freshness=expected, now=now, store=store)
    if reason:
        with pytest.raises((entities.StageValidationError, handoff.HandoffValidationError), match=reason):
    ...
    if case in {"rotation", "revoked", "compromised"}:
        with pytest.raises(handoff.HandoffValidationError, match="issuance"):
            handoff.sign_contract(value, key=keys["key-1"], issued_at=30, expires_at=60,
                                  freshness=FRESHNESS, store=store)


@pytest.mark.parametrize("case", ["unknown_authority", "nested_authority", "second_graph", "second_seam",
    "second_evidence", "unknown_major", "unsigned", "algorithm_downgrade", "schema_downgrade",
    "removed_signature", "severed_handoff_output", "duplicate_json", "nonfinite_json", "nonstring_key"])
def test_unknown_authority_field_and_signature_downgrade_refused(tmp_path, case):
    ...
    if case in {"unknown_authority", "second_graph", "second_seam", "second_evidence"}:
        field = {"unknown_authority": "approval", "second_graph": "graph_authority",
                 "second_seam": "seam_authority", "second_evidence": "evidence_authority"}[case]

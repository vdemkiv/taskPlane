...
def verify_forward_release_surface(root):
    ...
    if release.get("CURRENT_VERSION") != "2.17.26":
        errors.append("forward candidate is not exactly 2.17.26")
    if release.get("PREVIOUS_VERSION") != "2.17.20":
        errors.append("v2.17.20 is not preserved as the last released generation")
    if release.get("COMPATIBILITY_PREVIOUS_VERSION") != "2.17.25":
        errors.append("v2.17.25 is not preserved as compatibility N-1")
    ...
    required_doc_phrases = (
        ...
        "v2.17.25", "v2.17.26", "not released",
...
def _parser():
    ...
    p.add_argument("--verify-release-surface", action="store_true",
                   help="prove 2.17.26 manifests and both install archives")

...
INVENTORY = (
    ...
    *PYTEST_CHECKS,
    ...
    Check("refactor-metrics", (PYTHON, "scripts/ci_refactor_metrics.py")),
    Check("import-cycle-current", (PYTHON, "taskplane/import_cycles.py", "--root", ".", "--policy", "taskplane/tests/fixtures/import-cycles.json", "--report-only")),
...
def _ci_cell_commands(cell: Mapping[str, Any], root: Path) -> list[list[str]]:
    ...
    if kind == "quality-package":
        return [
            ...
            # Structural targets are reported follow-up work; see
            # docs/refactor-followups.md for the user's integration decision.
            [PYTHON, "scripts/ci_refactor_metrics.py"],
            [PYTHON, "taskplane/import_cycles.py", "--root", ".", "--policy",
             "taskplane/tests/fixtures/import-cycles.json", "--report-only"],

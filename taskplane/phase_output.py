"""Dependency-neutral phase-output observation for native lifecycle callers.
...
def is_phase_contract(contract: dict[str, Any]) -> bool:
    ...
    return bool(contract.get("worker_scoped") is True and
                lifecycle.get("schema") == WORKER_CONTRACT_LIFECYCLE_SCHEMA and
...
def _read_output(workspace: str, relative: str) -> bytes:
    ...
    limit = SUBMISSION_ARTIFACT_MAX_BYTES
...
def observe_terminal_output(workspace: str, contract: dict[str, Any], *,
                            workspace_fingerprint: str) -> dict[str, Any] | None:
    ...
    if (not isinstance(binding, dict) or binding.get("schema") != SUBMISSION_CONTRACT_SCHEMA or
            binding.get("required") is not True or binding.get("validation_rule") != VALIDATION_RULE or
            binding.get("workspace_fingerprint") != workspace_fingerprint or

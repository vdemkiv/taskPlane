...
class TestRepositoryFlowDocumentation(unittest.TestCase):
    ...
    def test_delivery_repository_preparation_is_conditional(self):
        ...
        product = self._read("skills/tp-product/SKILL.md")
        self.assertIn("Reuse readable local source", product)
        self.assertIn("repository preparation only when the requested source is unavailable", product)

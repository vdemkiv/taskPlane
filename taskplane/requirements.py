...
def _write_requirement_file(ws: str, entry: dict) -> None:
    ...
    with tp.file_lock(path):
        ...
        approval_line = (json.dumps(approval, sort_keys=True)
                         if approval else "—")
        amendment = entry.get("publication_sequencing_amendment")
        amendment_line = ("\n- publication_sequencing_amendment: " + json.dumps(amendment, sort_keys=True)
                          if amendment else "")
...
- product_signoff: {approval_line}{amendment_line}
...
def publication_sequence_requirement(before: dict, *, annotation: dict) -> dict:
    """Preview the one legacy publication sequencing change; no whole Product pass."""
    if before.get("product_signoff") or before.get("publication_sequencing_amendment"):
        raise ProductSignoffError("publication sequencing requires the original unsigned legacy requirement")
    after = json.loads(json.dumps(before))
    rows = after.get("acceptance") or []
    matches = [i for i, row in enumerate(rows) if isinstance(row, str) and row.startswith("FP-AC17 J6:")]
    old = "Prove PR-based delivery and separately authorized publication;"
    new = ("Prove the real pre-merge J6 prefix through PR-based delivery with review and CI mandatory; "
           "publication remains an explicit pending post-merge obligation requiring separate current "
           "authority, the merged candidate, exact artifact bytes and destination, and all applicable "
           "freshness/signing checks; no publication pass or authority is supplied by this sequencing change;")
    if len(matches) != 1 or rows[matches[0]].count(old) != 1:
        raise ProductSignoffError("publication sequencing does not match the exact original FP-AC17 clause")
    rows[matches[0]] = rows[matches[0]].replace(old, new)
    after["publication_sequencing_amendment"] = json.loads(json.dumps(annotation))
    if not product_dor(after)["passed"]:
        raise ProductSignoffError("publication sequencing retains unmet Product DoR requirements")
    return after


def apply_publication_sequence(ws: str, *, before: dict, after: dict) -> dict:
    """CAS in the incumbent requirement index; repair projection on exact replay.

    The loop owner must validate the human packet and journal before calling.
    This preserves legacy status and does not renew a Product sign-off.
    """
    if publication_sequence_requirement(before,
            annotation=after.get("publication_sequencing_amendment")) != after:
        raise ProductSignoffError("publication amendment changed unrelated requirement fields")
    with _index_lock(ws):
        idx = load_index(ws)
        current = next((row for row in idx["requirements"] if row.get("id") == before.get("id")), None)
        if current not in (before, after):
            raise ProductSignoffError("requirement changed concurrently during publication amendment")
        if current == before:
            current.clear()
            current.update(json.loads(json.dumps(after)))
            _save_index(ws, idx)
    _write_requirement_file(ws, after)
    return after


def publication_coverage_tasks(tasks: list, requirement_lookup, *, require_passed: bool = False) -> list:
    """Project the exact amended reference; never mutate a historical task/pass.

    Completed contributors still own their original work. Only the existing
    remaining J6 journey owner can provide passed ownership of the new clause.
    The normal coverage owner and all substantive evidence gates still run.
    """
    projected = json.loads(json.dumps(tasks))
    for task in projected:
        requirement = requirement_lookup(task.get("req")) or {}
        annotation = requirement.get("publication_sequencing_amendment")
        if not annotation:
            continue
        original = annotation.get("original_criterion")
        before = json.loads(json.dumps(requirement))
        before.pop("publication_sequencing_amendment")
        indices = [i for i,s in enumerate(before.get("acceptance", [])) if str(s).startswith("FP-AC17 J6:")]
        if len(indices) != 1 or not isinstance(original, str):
            raise ProductSignoffError("publication acceptance reference is ambiguous")
        effective = before["acceptance"][indices[0]]
        before["acceptance"][indices[0]] = original
        if publication_sequence_requirement(before, annotation=annotation) != requirement:
            raise ProductSignoffError("publication acceptance reference mapping changed")
        for key in ("criteria", "acceptance_refs"):
            if key in task:
                task[key] = [effective if value == original else value for value in task[key]]
        if require_passed:
            refs = task.get("acceptance_refs")
            if refs is None:
                explicit = task.get("criteria") or []
                refs = [v for v in explicit if v in requirement["acceptance"]] if explicit else list(requirement["acceptance"])
            task["acceptance_refs"] = [v for v in refs if v != effective]
            if task.get("id") == annotation["acceptance_owner"]:
                task["acceptance_refs"].append(effective)
    return projected

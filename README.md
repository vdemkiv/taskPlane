...
| Version | Highlights |
| --- | --- |
| **v2.17.3** | **Standalone reviews now ask explicitly for dynamic validation and deliver one interactive inline dashboard.** The execution preflight recommends dynamic checks, names discovered commands and missing dependency installation, and never silently downgrades to static review. Codex dashboard controls use the native follow-up bridge with exact engine receipt prompts; the dependency graph remains embedded instead of becoming a duplicate artifact. Collection validates every leased result in one pass and returns all producer-owned schema repairs as one batch, eliminating the one-slot-at-a-time retry loop without weakening provenance. |
| **v2.17.2** | **Completed review publication no longer strands its revision reservation after a hard process death.** An idempotent collect verifies the completed revision is still canonical before releasing only that run's dead exact-owner reservation; other runs and live owners remain protected, and the next revision can publish normally. |
| **v2.17.1** | **Remote PR review permission and lens handoff are now mechanically resumable.** A denied external-storage preflight persists its exact action locally, and repeated `review start` calls remain paused until an explicit human `review resume`; approval can no longer be inferred from a later successful command. ReviewKernel creates every leased-result parent before dispatch, resolves native Codex child writes back to the sealed managed checkout before containment screening, lets exact run-id collection pass the child-contract union, and releases only completed producer slots after submission. Models no longer need to run manual `mkdir`, `touch`, retry-wave, or broad contract-clear workarounds. |
| **v2.17.0** | **Lens aggregation now trusts the sealed artifact contract instead of treating optional host telemetry as the result itself.** Exact leased paths, immutable identities, the canonical schema, complete lens coverage, and finding/verdict consistency remain mandatory; a present host receipt is still checked strictly, but its absence no longer discards a valid review. Each collected result records a durable validation artifact and exposes whether host provenance was observed. The validation matrix is also leaner: Python 3.12 owns the authoritative pytest suite, Python 3.10/3.11 retain focused compatibility coverage, and one public unittest canary replaces a second partial execution of the same tests. |

...
if TYPE_CHECKING or __package__:
    from . import loop, phase_handoff, phase_inputs, taskplane_lite as kernel
else:
    import loop
...
def selected_json(workspace: str, handoff: dict[str, Any], kind: str) -> dict[str, Any]:
    ...
    if len(refs) != 1 or refs[0]["media_type"] != "application/json":
        raise phase_inputs.PhasePickupError(
    ...
    if not isinstance(value, dict):
        raise phase_inputs.PhasePickupError("authoring-invalid", f"sealed {kind} must be an object")
    return value
...
def validate_native_rows(tasks: object) -> None:
    ...
    if not isinstance(tasks, list) or any(not isinstance(row, dict) for row in tasks):
        raise phase_inputs.PhasePickupError("authoring-invalid", "native Plan tasks must be object rows")
...
validate_obligation_ownership = phase_inputs.validate_obligation_ownership
...
def project_tasks(plan: dict[str, Any], handoff: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if len({row["id"] for row in native}) != len(native):
        raise phase_inputs.PhasePickupError("authoring-invalid", "Plan task identities overlap")
    pending = list(native)
    ...
    while pending:
        ...
        if ready is None:
            raise phase_inputs.PhasePickupError("dependency-unmet", "Plan tasks have missing or cyclic dependencies")
...
def selected_inputs(workspace: str, handoff: dict[str, Any]) -> dict[str, Any]:
    ...
    if (requirement.get("id") != rid or requirement.get("acceptance") !=
        ...
        raise phase_inputs.PhasePickupError("scope-widened", "sealed Plan inputs disagree on the requirement")
    ...
    if required_contracts != {row["id"] for row in handoff["contracts"]}:
        raise phase_inputs.PhasePickupError("scope-widened", "sealed requirement contracts differ")
    ...
    if actual != handoff["design"]["artifact"]:
        raise phase_inputs.PhasePickupError("authoring-invalid", "canonical Design differs from selected authority")
    ...
    if strategy is not None:
        if not isinstance(strategy, dict) or not isinstance(strategy.get("path"), str):
            raise phase_inputs.PhasePickupError("authoring-invalid", "Design strategy authority is malformed")
        ...
        if actual_strategy not in handoff["selected_artifacts"]:
            raise phase_inputs.PhasePickupError("authoring-invalid", "Design strategy bytes are not sealed inputs")
    ...
    for dependency in requirement.get("depends_on") or []:
        ...
        if record.get("id") != dependency:
            raise phase_inputs.PhasePickupError("dependency-unmet", "selected requirement dependency is foreign")
    ...
    if not isinstance(history, list):
        raise phase_inputs.PhasePickupError("receipt-lineage", "selected Plan is missing sealed replan history")
...
def validate_output(workspace: str, handoff: dict[str, Any], plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if plan.get("replan_history") != inputs["replan_history"]:
        raise phase_inputs.PhasePickupError("receipt-lineage", "Plan must preserve sealed replan history")
    ...
    if json.loads(plan_bytes.decode("utf-8")) != plan:
        raise phase_inputs.PhasePickupError("artifact-integrity", "Plan argument differs from canonical output")
    ...
    if errors:
        raise phase_inputs.PhasePickupError("authoring-invalid", "Plan completion refused: " + "; ".join(errors))
    ...
    if prior is not None:
        ...
        for old in prior["tasks"]:
            if completed_acceptance.intersection(_acceptance_ids(old, handoff)) and (
                ...
                raise phase_inputs.PhasePickupError("receipt-lineage", "Plan rewrites an already completed task contract")
    return tasks

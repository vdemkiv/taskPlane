...
1. **Deep-review floors scale with attributable risk.** Documentation-only and
   simple low-risk changes dispatch exactly one risk-selected deep review lens.
   Substantive or risky changes dispatch architecture, code-quality, security,
   and QA as four mandatory deep floors. Cached failures, early blockers, or
   provisional publication cannot replace the deep slot or slots required for
   the change's risk class. **Verify:** documentation-only, simple low-risk,
   substantive, risky, mixed, empty-map, and early-blocker fixtures assert exact
   risk classification, lens selection, and slot conservation.

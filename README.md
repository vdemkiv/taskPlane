...
| **v2.8.0** | **The dependency graph now describes the codebase it reviews — measured, not asserted.** taskplane had a cost meter and a yield meter and no ACCURACY meter for the artifact every review depends on, so the graph drifted without anyone noticing. A new harness scores the real scanner against hand-authored ground truth over four repo profiles and found four defects, all fixed: module ids come from your build manifests where you declare them, so a monorepo module is `@acme/ui` rather than an invented `ui`; a nested `src/` no longer renames a module after a convention or merges two sibling apps into one node; markdown skills, agents, lenses, SQL, IaC and CI are first-class nodes with their files, so a repo whose product is not source code stops being invisible; and components that talk by NAMING each other finally have edges. Corpus edge recall went 33% → 100% (same-language imports), 40% → 100% (build manifests) and 0% → 100% (declared data); this repo went from 6 modules and 4 internal edges to 28 and 120. A repo can also declare in `components.yaml` which trees are not its product code. From the same whole-codebase review: a `--all` review no longer fans out 26 subagents under a cap of 8 (the overflow is demoted to inline, never dropped); content detectors stopped firing on documentation that merely DESCRIBES what they look for; a cited suite result is bounded in time and disclosed at the gate instead of only in the trace; release archives name their source commit and refuse a dirty tree; the dashboard reports a render failure instead of silently omitting itself; review prose is no longer auto-committed into a team store; trace rotation stopped destroying its own archive; and four verified contract-screener bypasses are closed. 1,735 tests, every fix mutation-tested. |
| **v2.7.4** | **Windows portability — all four classes closed, each reproducible on Linux.** taskplane had never actually worked on Windows and CI could not say so: the failures lived only on an advisory leg that times out mid-suite and blocks nothing. Encoding (`open()` and printed arrows used the host codec — cp1252 there, UTF-8 here) now gates via a C-locale leg that reproduces the class in two minutes: 314 failures before, 0 after. Separators were a governance defect — every path in a Windows workspace resolved to `ESCAPES:`, so the contract screener refused a worker's own in-scope file; repo-path arithmetic moved to `posixpath` with a static ratchet against regression. Detector reads normalize CRLF so a Windows checkout routes identically, and `rmtree` clears the read-only bit git leaves on `.git/objects`. A second pass took the leg from 90 failures to 6, and those six named the worst bug of the set: `os.kill(pid, 0)` is not a liveness probe on Windows — it sends Ctrl+C to the console group, which is what had been truncating every CI run, and it never measured liveness, so an orphaned contract was never released there. With that gone the leg ran all 1,558 tests for the first time — the honest residue was 23, not the 6 a truncated run had implied, and most of it was one shape: `shlex.split` eats backslashes in POSIX mode, so every path-shaped screen on Windows was matching a mangled string. A fourth pass on the complete failure list found four more host-divergent defects, including JSON artifacts written with CRLF — which matters because taskplane fingerprints and byte-compares them — and a regression-gate containment check that matched nothing there. 1,558 tests. |

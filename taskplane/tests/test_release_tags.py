...
def test_current_candidate_has_an_explicit_nonrelease_predecessor():
    disposition = gate.NOT_RELEASED["2.18.6"]
    assert disposition["superseded_by"] == "2.18.7"
...
def test_219_rollback_candidates_are_not_fictional_releases(tmp_path, damage):
    ...
    repo.release("2.23.0")
    repo.release("2.23.1")
    versions = ["1.0.0", "2.19.0", "2.19.1", "2.20.0", "2.23.0", "2.23.1"]
    ...
    if damage is None:
        ...
        assert gate.NOT_RELEASED["2.23.0"]["superseded_by"] == "2.23.1"
    else:

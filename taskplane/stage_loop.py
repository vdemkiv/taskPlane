...
def _stage_request(_ports, request: object) -> dict:
    ...
    try:
        return _ports.json.loads(_ports.json.dumps(
...
def authorized_run_revision(_ports, ws, manifest, baseline, current):
    ...
    tasks = [task for task in state.get("tasks") or [] if task_id is None or task.get("id") == task_id]
    scopes = sorted({path for task in tasks for path in task.get("scope") or []})
    if not scopes or _ports.tp._run(["git", "merge-base", "--is-ancestor", baseline, current], cwd=ws).returncode:
        return False
    changed = _ports.tp._run(["git", "diff", "--name-only", "-z", baseline, current, "--"], cwd=ws)
    return changed.returncode == 0 and all(_ports.tp.writable_target(path, scopes, ws)
        for path in changed.stdout.split("\0") if path)


def _current_stage_authority(_ports,
    ...
    if live_revision and live_revision != "unknown":
        ...
        if live_revision != baseline and authorized_run_revision(_ports, ws, manifest, baseline, live_revision):
...
def _stage_lifecycle(_ports, ws: str, store: object, manifest: Mapping[str, object],
                     authority: object):
...
def _indexed_stage(_ports, store: object, manifest: Mapping[str, object],
    ...
    if not isinstance(head, _ports.Mapping) or not isinstance(
            head.get("object"), _ports.Mapping):
...
def _verified_stage_handoff(_ports, lifecycle: object, store: object,
...
def _stage_dispatch(_ports, store: object, lifecycle: object,
    ...
    handoff = _ports._verified_stage_handoff(
...
def _preflight_stage_dispatch(_ports, stage: dict, handoff: dict,
...
def _stage_bootstrap_pristine_root(_ports,
    ...
    if not isinstance(raw_authority, _ports.Mapping) or \
            set(raw_authority) != _ports._STAGE_ROOT_AUTHORITY_FIELDS:
    ...
    if root_authority.get("schema") != _ports._STAGE_ROOT_AUTHORITY_SCHEMA or \
...
def task_phase_state(_ports, ws, state, task_id=None):
    ...
    if selected is None or state is None:
        return state
    indexes = [index for index, task in enumerate(state.get("tasks") or []) if task.get("id") == selected]
    if len(indexes) != 1:
    ...
    if state.get("parallel"):
        view["step"] = task.get("phase_step") or {
            "pending": "execute", "running": "execute", "built": "evaluate",
...
def _stage_loop_context(_ports,
    ...
    if foreground is None and isinstance(state, _ports.Mapping):
        ...
        task_bindings = (bindings.get(task_id)
                         if isinstance(bindings, _ports.Mapping) else None)
...
def _stage_loop_scope(_ports, scope_paths: object,
...
def _stage_loop_completion_reference(_ports,
    ...
    try:
        detached = _ports.json.loads(_ports.json.dumps(
...
def _before_stage_output_component_open(_ports,
...
def _stage_loop_open_directory_no_follow(_ports, path: str) -> int:
    ...
    flags = _ports.os.O_RDONLY | _ports.os.O_DIRECTORY | _ports.os.O_NOFOLLOW | \
...
def _stage_loop_read_output_no_follow(_ports,
    ...
    try:
        ...
        stable = (
            _ports.stat.S_ISREG(after.st_mode) and before.st_dev == after.st_dev and
...
def _stage_loop_trusted_output_workspace(_ports,
    ...
    if not isinstance(submission, _ports.Mapping) or not str(
    ...
    expected = _ports.os.path.abspath(_ports.runtime_storage.task_worktree_path(
    ...
    if not isinstance(current_locator, _ports.Mapping) or \
            not isinstance(source_locator, _ports.Mapping) or \
            not isinstance(registration, _ports.Mapping) or any(
            ...
            _ports.os.path.abspath(str(registration.get("path") or "")) != claimed or \
            _ports.os.path.abspath(str(
...
def _stage_loop_managed_evidence_paths(_ports,
    ...
    allowed = {}
    for expected in _ports.runtime_storage.submission_evidence_paths(
        ...
        try:
            contained = any(_ports.os.path.commonpath((root, canonical)) == root
                            for root in roots)
        except ValueError:
            contained = False
        if not contained:
...
def _stage_loop_decision_completion(_ports,
    ...
    return {
        ...
        "outcome": str(outcome), "workspace_revision": _ports.tp.git_head(ws),
        **detached,
        "_stage_output": {
...
def _stage_loop_completion_outputs(_ports,
        lifecycle: object, completion: Mapping[str, object]) \
    ...
    if not isinstance(sources, list) or \
            len(sources) > _ports._STAGE_OUTPUT_MAX_SOURCES:
    ...
    for index, raw in enumerate(sources):
        ...
        if not logical:
            logical = (relative if not _ports.os.path.isabs(supplied) else
                       f"evidence/{index:03d}-{_ports.os.path.basename(path)}")
        logical = logical.replace("\\", "/")
        logical_components = logical.split("/")
        if _ports.os.path.isabs(logical) or not logical or any(
                component in {"", ".", ".."}
    ...
    if build is not None:
        ...
        sha = str(build.get("target_commit") or
                  _ports.tp.git_head(source_workspace) or "").strip()
        ...
        identity = _ports.runtime_storage.resolve_repository_identity(
            source_workspace)
        target_material = {
...
def _stage_loop_transition_operation_material(_ports,
...
def _stage_loop_transition(_ports,
    ...
    if completion is None and isinstance(
            state.get("_stage_completion"), _ports.Mapping):
    ...
    if from_kind is None or (
        ...
        return None
    if ((from_step, to_step) in {("design", "design_approval"), ("plan", "plan_approval"),
                               ("em", "signoff")} and
            _ports._phase_bridge_context(ws, state) is not None):
    ...
    if from_kind in {"build", "evaluate"} and source_task_id:
        source_state["current_task"] = next(index for index, row in enumerate(state["tasks"])
            if row["id"] == source_task_id)
    ...
    operation_material = _ports._stage_loop_transition_operation_material(
        ...
        predecessor_stage_id=(
            stage.get("stage_id") if isinstance(stage, _ports.Mapping) else None),
    ...
    prior = operations.get(operation_id) if isinstance(operations, _ports.Mapping) \
        else None
    ...
    if completion:
        completion, selected_artifacts, target, commit = \
            _ports._stage_loop_completion_outputs(lifecycle, completion)
        evidence = [_ports._stage_loop_completion_reference(
    ...
    successor = stage_entities.create_stage(
        ...
        selected_artifacts=selected_artifacts,
        budget=dict(stage.get("budget") or {}), dependencies=sorted(set(dependencies)),
    ...
    if task is not None and to_kind in {"build", "evaluate"} and (
            not state.get("parallel") or from_kind in {"build", "evaluate"}):
...
def stage_history(_ports, ws: str, run_id: str, *, cursor: object = None,
                  limit: int | None = None) -> dict:
    """Public bounded read helper used by non-CLI host adapters."""
    if limit is None:
        limit = _ports.STAGE_HISTORY_MAX_ITEMS
    try:
        return _ports._stage_history(_ports._stage_store(ws, run_id), run_id, {
...
def _stage_history(_ports, store: object, run_id: str, request: Mapping[str, object]) \
        -> dict:
    manifest = store.load(run_id)
    if manifest.get("schema") != "taskplane.run/v4":
    ...
    if isinstance(raw_limit, bool) or not isinstance(raw_limit, int) or \
            not 1 <= raw_limit <= _ports.STAGE_HISTORY_MAX_ITEMS:
        raise ValueError(
            f"history limit must be 1..{_ports.STAGE_HISTORY_MAX_ITEMS}")
    ...
    lineage = [dict(row) for row in manifest.get("lineage") or []
               if isinstance(row, _ports.Mapping) and
    ...
    with _ports.mutate(ws) as state:
        ...
        if receipt is not None:
            if (current["stage_operations"].get(receipt["operation_id"]) != receipt or
                    current["active_stage_projection"]["foreground_stage_id"] != stage["stage_id"]):
...
def _start_or_reuse_stage(_ports, ws, action, data, store, run_id, stage_entities, lifecycle):
    ...
    verified_handoff = _ports._verified_stage_handoff(
        lifecycle, store, current, stage)
    if action == "reuse":
        ...
        producer_id = (producer.get("stage_id")
                       if isinstance(producer, _ports.Mapping) else None)
        selected_producers = [
        ...
        reuse_authorization = (authorization.get(
            "nonconsumable_reuse")
            if isinstance(authorization, _ports.Mapping) else None)
        actor = data.get("actor")
        if not isinstance(reuse_authorization, _ports.Mapping) or \
    ...
    if action == "start":
        _ports._project_bound_stage_start(ws, store, lifecycle, stage,
    ...
    return {"schema": _ports.STAGE_COMMAND_SCHEMA, "command": action,
...
def _resume_stage(_ports, ws, action, data, store, run_id, stage_entities, lifecycle):
    current = store.load(run_id)
    active_stage = _ports._indexed_stage(
...
def stage_command(_ports, ws: str, command: str, request: object) -> dict:
    ...
    if action not in allowed:
        return _ports._stage_command_error(
    ...
    try:
        ...
        if action in {"start", "reuse"}:
            return _start_or_reuse_stage(_ports, ws, action, data, store, run_id, stage_entities, lifecycle)
        ...
        for child_id in sorted(str(value) for value in child_heads):
            ...
            dispatches.append(_ports._stage_dispatch(
...
def dispatch_authority_error(_ports, state):
    ...
    if not isinstance(authority, _ports.Mapping) or set(authority) != _ports._STAGE_ROOT_AUTHORITY_FIELDS:
    ...
    if (authority.get("schema") != _ports._STAGE_ROOT_AUTHORITY_SCHEMA or
            authority.get("run_id") != state.get("run_id") or
            authority.get("requirement_id") != state.get("requirement_id") or
            content_fingerprint(material) != authority.get("fingerprint")):
        return "phase dispatch root authority is invalid"
    return None


def _stage_native_init_authority(_ports,
    ...
    session_id = str(
        _ports.os.environ.get("TASKPLANE_SESSION_ID") or
        _ports.os.environ.get("CODEX_THREAD_ID") or
        _ports.os.environ.get("CLAUDE_SESSION_ID") or "").strip()
    if not session_id:
        raise ValueError(
    ...
    if manifest.get("schema") != "taskplane.run/v4" or any(
        ...
        raise ValueError(
            "phase initialization requires a current v4 run")
    repository = manifest.get("repository")
    if not isinstance(repository, _ports.Mapping) or \
...
def _verified_stage_loop_wave_split(_ports,
    ...
    expected_bindings = ({str(key): str(value)
                         ...
                         if isinstance(known_bindings, _ports.Mapping) else {})
    ...
    for raw in operations.values():
        ...
        if not isinstance(parent_head, _ports.Mapping) or not isinstance(
                child_heads, _ports.Mapping) or not isinstance(
                receipt_projection, _ports.Mapping):
        ...
        if not isinstance(receipt_lineage, list) or any(
                not isinstance(row, _ports.Mapping) or
...
def _persist_stage_loop_wave_bindings(_ports,
    ...
    with _ports.mutate(ws) as locked:
        ...
        if any(
                ...
                (locked_tasks[task_id].get("status") == "running" and not (
                    isinstance(existing_table, _ports.Mapping) and
                    isinstance(existing_table.get(task_id), _ports.Mapping) and
                    existing_table[task_id].get("build") == bindings[task_id]))
                for task_id in task_ids):
            raise ValueError("stage-native wave changed during split binding")
        current = _ports._verified_stage_loop_wave_split(
...
def _stage_loop_wave_dispatches(_ports,
    ...
    known = {
        ...
        for binding in [table.get(str(task["id"]))
                        if isinstance(table, _ports.Mapping) else None]
    ...
    recovered = _ports._verified_stage_loop_wave_split(
    ...
    if len(ready) == 1:
        ...
        return {task_id: _ports._stage_loop_dispatch(
    ...
    terminalized_at = _ports.time.strftime(
        "%Y-%m-%dT%H:%M:%SZ", _ports.time.gmtime())
    ...
    for ordinal, task in enumerate(ready):
        ...
        child_specs.append({
            ...
            "selected_artifacts": selected,
            "dependencies": ["child:" + str(ordinals[str(dependency)]) for dependency in task.get("deps") or []],
...
def _stage_loop_gate_completion(_ports,
    ...
    if step == "pm":
        ...
        record = _ports.reqs.get_requirement(ws, requirement_id) \
        ...
        result["design_fingerprint"] = state.get("design_fingerprint") or \
            _ports._design_evidence_fingerprint(ws)
        sources.extend([
        ...
        result["signoff_fingerprint"] = (
            ...
            if isinstance(signoff, _ports.Mapping) and hasattr(
                _ports.review_session_engine, "signoff_evidence_fingerprint")
            else None)
    ...
    if step in {"execute", "fix"} and not any(
            ...
            for source in sources if isinstance(source, _ports.Mapping)):
    ...
    result["_stage_output"] = {
        ...
        **({"managed_evidence_step": str(step)}
           if isinstance(submission, _ports.Mapping) and
...
def archive(_ports, ws: str, *, by: str) -> dict:
    ...
    _ports.tp.atomic_write_json(destination + '.receipt.json', {'schema': 'taskplane.run-archive/v1', 'run_id': locator['run_id'], 'actor': str(by).strip(), 'archived_at': int(_ports.time.time())})
    _ports.os.rename(source, destination)
    return {'archived': True, 'run_id': locator['run_id'], 'locator': destination, 'evidence_retained': True}
...
def resume(_ports, ws: str) -> dict:
    ...
    if state is None:
        return {'schema': 'taskplane.loop-resume/v1', 'status': 'not_started', 'next_action': 'initialize_run', 'command': 'loop init'}
    if (refusal := _ports._run_schema_refusal(ws)):
        return {'schema': 'taskplane.loop-resume/v1', 'status': 'refused', **refusal}
    tasks = state.get('tasks') or []
    return {'schema': 'taskplane.loop-resume/v1', 'status': 'resumable', 'read_only': True, 'run_id': state['run_id'], 'goal': state['goal'], 'step': state['step'], 'requirement_id': state.get('requirement_id'), 'spec_path': state.get('spec_path'), 'state_path': _ports._loop_path(ws), 'current_task': state.get('current_task'), 'tasks': [{'id': task['id'], 'scope': task.get('scope') or [], 'status': task.get('status')} for task in tasks], 'design_fingerprint': state.get('design_fingerprint'), 'plan_fingerprint': state.get('plan_fingerprint'), 'next_action': 'resolve_current_action', 'command': 'loop next', 'authority': 'revalidate durable authorization before effects'}

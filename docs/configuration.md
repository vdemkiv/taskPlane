...
| `TASKPLANE_PUBLISH_REVIEW` | *(unset — withheld)* | `1`/`true`/`yes`/`on` allows model-authored review artifacts (`.em-review/findings.json`, `report.md`, `retro.md`, and the rendered `dashboard.html`, which embeds them) to be copied into an **in-repo** store (`.taskplane-kb/`, the team/enterprise plan) by the automatic gate snapshot. Unset, they stay in the checkout and the snapshot NAMES what it withheld. PRIVACY.md states that publishing to a shared store is a deliberate human act and that `.em-review/` stays local on both plans; this variable is that act. An **external** (personal) store is outside the repo and is unaffected — it publishes everything, as before. | No — unset it and less leaves the checkout. |
...
| `TASKPLANE_RUNNABILITY` | *(unset — probe on)* | `off`/`0`/`false`/`no` skips the build/test **runnability probe** that `tp lens dispatch` runs before composing briefs. The probe answers one question about the CHECKOUT — would `go test ./...` / `npm test` / `pytest` get off the ground here — using a bounded, cheap subcommand (`go list ./...`, `cargo metadata --offline`, `import pytest`), never the suite itself, and states the verdict in every dispatched brief plus the wave board. It exists because on `aws/karpenter-provider-aws#9464` six lens agents were dispatched in parallel and all six independently spent actions discovering that `go test` could not run in that sandbox: one environment fact, paid for six times. The answer is cached in `.taskplane/runnability.json`, keyed by the manifests that decide it and the `PATH` that resolves the toolchain, so a whole wave shares one probe while installing the missing toolchain mid-review re-probes. | No — it is information, never a gate: no screener, contract, or gate consults it (pinned by `test_runnability_probe.py::TestItIsInformationNotEnforcement`). Setting it `off` only makes agents rediscover the fact themselves. |

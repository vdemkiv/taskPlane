...
def resolve_repository_family(workspace: str) -> dict:
    ...
    def git_path(*arguments: str) -> str | None:
        ...
        return os.path.realpath(value) if result.returncode == 0 and value else None

    worktree = git_path("rev-parse", "--path-format=absolute", "--show-toplevel") or next(
        (path for path in chain if os.path.exists(os.path.join(path, ".git"))), current
    )
    launcher = next(
        (
            os.path.realpath(os.path.join(path, ".taskplane", "codex-hook.py"))
            for path in chain
            if os.path.isfile(os.path.join(path, ".taskplane", "codex-hook.py"))
        ),
        None,
    )
    if launcher is None:
        ...
        common = git_path("rev-parse", "--path-format=absolute", "--git-common-dir")
        candidate = (
            os.path.join(os.path.dirname(common), ".taskplane", "codex-hook.py") if common else None
        )
        if candidate and os.path.isfile(candidate):
            launcher = os.path.realpath(candidate)
    return {"schema": "taskplane.repository-family/v1", "worktree": worktree, "launcher": launcher}
...
class RepositoryIdentity:
    ...
    @property
    def key(self) -> str:
        readable = "-".join(value for value in (self.host, self.owner, self.name) if value)
...
def identity_from_remote(remote: str, *, workspace: str | None = None) -> RepositoryIdentity:
...
def resolve_repository_identity(workspace: str, *, remote: str | None = None) -> RepositoryIdentity:
    ...
    return RepositoryIdentity(
        repo_id=f"local/{name.lower()}/{digest}",
        kind="local",
        host=None,
        owner=None,
        name=name,
...
def resolve_layout(
    identity: RepositoryIdentity, *, run_id: str, home: str | None = None
) -> StorageLayout:
...
def _storage_file_lock(path: str, *, timeout: float = 10.0):
    ...
    try:
        ...
        raise StorageIdentityError(f"could not open dashboard storage lock: {exc}") from exc
    try:
        import fcntl
    except ImportError:
        fcntl = None
    ...
    while True:
        try:
            ...
            if time.monotonic() >= deadline:
                raise StorageIdentityError("could not acquire dashboard storage lock")
            time.sleep(0.01)
            continue
        except OSError as exc:
            raise StorageIdentityError(f"could not acquire dashboard storage lock: {exc}") from exc
        try:
    ...
    try:
        ...
        try:
            ...
            if observed_owner != owner:
                raise StorageIdentityError(
                    "could not release dashboard storage lock: ownership is ambiguous"
                )
            ...
            raise StorageIdentityError(f"could not release dashboard storage lock: {exc}") from exc
...
def _selection_path(checkout: str) -> str:
    return _checked_binding_path(
        os.path.join(os.path.dirname(_locator_path(checkout)), "execution-storage.json")
    )
...
def _load_storage_selection(checkout: str) -> dict | None:
    ...
    if (
        ...
        or set(value) != {"schema", "checkout", "home", "previous_binding"}
        or value.get("schema") != "taskplane.execution-storage-selection/v1"
        or value.get("checkout") != root
        or value.get("home") != project_taskplane_home(root)
    ):
    ...
    if previous is not None:
        if (
            ...
            or not _FINGERPRINT.fullmatch(str(previous.get("locator_fingerprint")))
            or not _FINGERPRINT.fullmatch(str(previous.get("manifest_fingerprint")))
            or previous.get("locator_path") != _locator_path(root)
        ):
        ...
        expected = _selection_archive_path(value["home"], previous["locator_fingerprint"])
        ...
        if (
            ...
            or _binding_fingerprint(archive.get("locator")) != previous["locator_fingerprint"]
            or _binding_fingerprint(archive.get("manifest")) != previous["manifest_fingerprint"]
        ):
...
def _selection_archive_path(home: str, fingerprint: str) -> str:
    ...
    return _confined_stage_path(home, "storage-selections", digest + ".json", leaf_kind="file")


def write_workspace_locator(
    checkout: str, *, identity: RepositoryIdentity, layout: StorageLayout, run_id: str
) -> str:
...
def load_workspace_locator(checkout: str) -> dict | None:
    ...
    if not isinstance(value, dict) or value.get("schema") != "taskplane.workspace/v1":
    ...
    if primary != root and (
        not isinstance(value.get("task_id"), str) or not value["task_id"].strip()
    ):
    ...
    for item in paths.values():
        if (
            ...
            or os.path.commonpath((home, os.path.realpath(item))) != home
        ):
...
def bind_workspace_taskplane_home(
    ...
    expected = str(locator["home"]) if locator else str(selection["home"]) if selection else None
    if expected is None:
        return None
...
def _unused_preflight_manifest(checkout: str, locator: dict) -> dict:
    ...
    try:
        _confined_stage_path(
            locator["home"], "runs", locator["run_id"], "manifest.json", leaf_kind="file"
        )
        manifest = run_store_module.RunStore(home=locator["home"]).inspect(locator["run_id"])
    except (OSError, ValueError, StorageIdentityError, run_store_module.RunStoreError) as exc:
        raise StorageIdentityError(f"{refusal}: prior run could not be inspected") from exc
    ...
    if (
        ...
        or manifest.get("status") not in {"preflight", "ready", "awaiting_user", "waiting_external"}
        or manifest.get("contract") != {"status": "inactive", "task_id": None}
        or any(
            manifest.get(key)
            for key in ("stage_heads", "lineage", "stage_operations", "stage_journal_outbox")
        )
        or manifest.get("paths") != locator.get("paths")
        or repository.get("checkout") != checkout
        ...
        raise StorageIdentityError(f"{refusal}: existing run is not an unused repository preflight")
            ...
            for name in names + filenames:
                ...
                if (
                    ...
                    in {
                        "loop.json",
                        "tracks.json",
                        STAGE_EXECUTION_ROOT_CLAIM,
                        STAGE_EXECUTION_ATTEMPT_CLAIM,
                    }
                    or "submission" in name.lower()
                    or (relative[0] == "stages" and os.path.isfile(path))
                ):
                    raise StorageIdentityError(
    ...
    if os.path.isdir(history_root):
        ...
        if os.path.lexists(tracks_path):
            if os.path.islink(tracks_path):
                raise StorageIdentityError(
                    f"{refusal}: active track catalog uses an unsafe symlink"
                )
            try:
                ...
                raise StorageIdentityError(
                    f"{refusal}: active track catalog is unreadable"
                ) from exc
        ...
        for directory, names, filenames in os.walk(
            ...
            if history_run == locator["run_id"] or (
                ...
                raise StorageIdentityError(f"{refusal}: current binding has legacy execution state")
    return manifest


def select_project_execution_storage(
    workspace: str, *, environment: MutableMapping[str, str] | None = None
) -> dict:
    ...
    if locator and locator["home"] == home:
        if environment is not None:
            environment["TASKPLANE_HOME"] = home
        return {
            "schema": "taskplane.execution-storage-selection/v1",
            "status": "already_selected",
            "checkout": root,
            "home": home,
            "previous_binding": None,
            "prior_history_guard": False,
        }
    ...
    if locator:
        ...
        archive = {
            "schema": "taskplane.execution-storage-archive/v1",
            "locator": locator,
            "manifest": manifest,
        }
        ...
        if (
            _binding_fingerprint(_unused_preflight_manifest(root, locator))
            != previous["manifest_fingerprint"]
            or _binding_fingerprint(load_workspace_locator(root)) != fingerprint
        ):
...
def bind_hook_taskplane_home(
    checkout: str, environment: MutableMapping[str, str], *, hook_path: str | None = None
) -> str:
...
def managed_path_allowed(checkout: str, path: str) -> bool:
    ...
    return any(
        ...
        for root in locator["paths"].values()
    )


def _path_component(value: object, pattern: re.Pattern[str], label: str) -> str:
...
def _confined_stage_path(home: str, *parts: str, leaf_kind: str) -> str:
    ...
    for index, part in enumerate(relative):
        ...
        try:
            ...
            raise StorageIdentityError(f"stage path is not safely inspectable: {exc}") from exc
        if stat.S_ISLNK(mode):
        ...
        if not is_leaf and not stat.S_ISDIR(mode):
            raise StorageIdentityError("stage path ancestor is not a directory")
...
def stage_object_path_for_run(home: str, run_id: str, stage_id: str, fingerprint: str) -> str:
    ...
    return _confined_stage_path(
        home, "runs", run, "stages", "objects", stage, f"{digest}.json", leaf_kind="file"
    )


def _confined_directory_components(home: str, directory: str) -> tuple[str, list[str]]:
...
def _open_confined_directory(home: str, directory: str, *, create: bool) -> int:
    ...
    if (
        ...
        raise StorageIdentityError("stage storage needs no-follow dir-fd support")
    root, components = _confined_directory_components(home, directory)
    flags = os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW
    try:
        descriptor = os.open(root, flags)
    except OSError as exc:
        raise StorageIdentityError(f"taskPlane home is not safely inspectable: {exc}") from exc
    try:
        ...
        os.close(descriptor)
        raise StorageIdentityError(f"stage directory is not safely accessible: {exc}") from exc


def _create_claimed_directory(parent_fd: int, name: str, *, claim_name: str, payload: bytes) -> int:
    ...
    try:
        ...
        raise StorageIdentityError("stage execution root is not a safe directory") from exc
    ...
    try:
        try:
            ...
            if not created:
                raise StorageIdentityError("existing stage execution root has no exact claim")
            write_flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW
            try:
                claim_fd = os.open(claim_name, write_flags, 0o600, dir_fd=descriptor)
                ...
                raise StorageIdentityError(
                    "stage execution-root claim could not be created"
                ) from exc
            return descriptor
        except OSError as exc:
            raise StorageIdentityError("stage execution-root claim is not safely readable") from exc
        ...
        if existing != payload or metadata.st_size != len(payload):
            raise StorageIdentityError("stage execution root belongs to another claim")
        return descriptor
    except Exception:
        os.close(descriptor)
        raise
...
def _verify_execution_root_claim(directory_fd: int, *, claim_name: str, payload: bytes) -> None:
    """Verify exact private claim bytes through one pinned directory FD."""
    try:
        claim_fd = os.open(claim_name, os.O_RDONLY | os.O_NOFOLLOW, dir_fd=directory_fd)
    except OSError as exc:
        raise StorageIdentityError("stage execution-root claim is not safely readable") from exc
    with os.fdopen(claim_fd, "rb") as handle:
        metadata = os.fstat(handle.fileno())
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
            raise StorageIdentityError("stage execution-root claim is not a private regular file")
        existing = handle.read(len(payload) + 1)
    if existing != payload or metadata.st_size != len(payload):
        raise StorageIdentityError("stage execution root belongs to another claim")
...
def _reopen_execution_root_claim(
    home: str, path: str, pinned_fd: int, *, claim_name: str, payload: bytes
) -> None:
    ...
    try:
        ...
        if (pinned.st_dev, pinned.st_ino) != (current.st_dev, current.st_ino):
            raise StorageIdentityError("stage execution root changed during claim")
        _verify_execution_root_claim(current_fd, claim_name=claim_name, payload=payload)
    finally:
        os.close(current_fd)


def _root_claim(run_id: str, stage_id: str, execution_root_id: str) -> dict:
    return {
...
def _attempt_claim(run_id: str, stage_id: str, execution_root_id: str, attempt_id: str) -> dict:
    return {
...
def claim_stage_execution_root_for_run(
    home: str, run_id: str, stage_id: str, execution_root_id: str, attempt_id: str | None = None
) -> dict:
    """Create once or re-prove an exact isolated stage/attempt root claim."""
    run = validate_stage_path_id(run_id, "run id")
    stage = validate_stage_path_id(stage_id, "stage id")
    execution = validate_stage_path_id(execution_root_id, "execution root id")
    attempt = validate_stage_path_id(attempt_id, "attempt id") if attempt_id is not None else None
    ...
    try:
        stage_fd = _create_claimed_directory(
            executions_fd, stage, claim_name=STAGE_EXECUTION_ROOT_CLAIM, payload=root_payload
        )
    finally:
    ...
    try:
        ...
        if attempt is None:
            ...
            attempt_fd = _create_claimed_directory(
                stage_fd, attempt, claim_name=STAGE_EXECUTION_ATTEMPT_CLAIM, payload=attempt_payload
            )
            try:
...
def ensure_stage_object_parent_for_run(
    home: str, run_id: str, stage_id: str, fingerprint: str
) -> str:
...
def stage_execution_root_for_run(
    home: str, run_id: str, stage_id: str, attempt_id: str | None = None
) -> str:
...
def _stage_locator(checkout: str) -> tuple[dict, str]:
    ...
    locator = load_workspace_locator(root)
    if locator is None:
        raise StorageIdentityError("stage storage requires a canonical run locator")
    return locator, root


def _validate_stage_source_separation(locator: dict, checkout: str, path: str) -> None:
    if os.path.commonpath((checkout, path)) != checkout:
        return
    primary = str(locator["primary_checkout"])
    home = project_taskplane_home(primary)
    if locator["home"] != home or os.path.commonpath((home, path)) != home or path == home:
...
def stage_execution_root(checkout: str, stage_id: str, attempt_id: str | None = None) -> str:
...
def task_worktree_path(checkout: str, task_id: str) -> str:
    ...
    return os.path.join(
        ...
        "tasks",
        str(locator["run_id"]),
        _worktree_token(task_id),
    )


def task_worktree_reference(checkout: str, task_id: str) -> str:
    return (
        task_worktree_path(checkout, task_id)
        if load_workspace_locator(checkout)
        else f".tp-work/{task_id}"
    )


def task_worktree_registration_path(primary_checkout: str, task_id: str) -> str:
    locator = load_workspace_locator(primary_checkout)
    base = (
        os.path.join(locator["paths"]["state"], "worktree-registrations")
        if locator
        else os.path.join(
            os.path.realpath(primary_checkout), ".taskplane", "worktree-registrations"
        )
    )
...
def _git_common_dir(checkout: str) -> str | None:
    ...
    if not common:
        return None
    return os.path.realpath(common if os.path.isabs(common) else os.path.join(checkout, common))
...
def validate_task_worktree_registration(
    ...
    *,
    ...
    if supplied != worker or worker == primary or "\n" in worker:
        raise StorageIdentityError("managed worktree path uses an unsafe alias or symlink")
    ...
    if (
        ...
        or registration.get("schema") != "taskplane.managed-task-worktree/v1"
    ):
    ...
    if (
        ...
        or repository.get("repo_id") != identity.repo_id
        or registration.get("repository_key") != identity.key
        or worker_identity.repo_id != identity.repo_id
    ):
        raise StorageIdentityError("managed worktree repository changed")
    registered_primary = os.path.realpath(str(registration.get("primary_checkout") or ""))
    registered_worker = os.path.realpath(str(registration.get("path") or ""))
    if (
        registration.get("linked") is not True
        or registration.get("task_id") != str(task_id)
        or registered_primary != primary
        or registered_worker != worker
    ):
    ...
    resolved_target = _git_value(worker, "rev-parse", "--verify", f"{target}^{{commit}}")
    primary_tip = _git_value(primary, "rev-parse", branch_ref)
    if (
        resolved_target != target
        or head != target
        or primary_tip != target
        or registration.get("branch_tip") != target
    ):
        raise StorageIdentityError("managed task target commit and current HEAD differ")
    linked = _linked_worktree_record(primary, worker)
    if not linked or linked.get("HEAD") != target or linked.get("branch") != branch_ref:
        raise StorageIdentityError("managed linked-worktree identity changed")
    return registration


def load_task_worktree_registration(primary_checkout: str, task_id: str) -> dict | None:
    ...
    try:
        ...
        raise StorageIdentityError(f"managed worktree registration is unreadable: {exc}") from exc
    if not isinstance(value, dict) or value.get("schema") != "taskplane.managed-task-worktree/v1":
        raise StorageIdentityError("managed worktree registration is invalid")
    expected = os.path.realpath(task_worktree_path(primary_checkout, task_id))
    if (
        value.get("task_id") != str(task_id)
        or os.path.realpath(str(value.get("path") or "")) != expected
    ):
        raise StorageIdentityError("managed worktree registration identity mismatch")
    return value


def register_task_worktree(primary_checkout: str, worker_checkout: str, task_id: str) -> dict:
    ...
    if worker != expected:
        raise StorageIdentityError("worker checkout is outside its managed task worktree path")
    branch, tip = _worktree_branch_tip(worker)
    if not tip:
        raise StorageIdentityError("managed task worktree must have a recorded tip")
    identity = resolve_repository_identity(primary)
    worker_identity = resolve_repository_identity(worker)
...
def refresh_task_worktree_tip(primary_checkout: str, task_id: str) -> dict:
    ...
    if value is None:
        raise StorageIdentityError("managed task worktree registration is missing")
    ...
    worker_identity = resolve_repository_identity(worker)
    if (
        identity.repo_id != value.get("repository", {}).get("repo_id")
        or worker_identity.repo_id != identity.repo_id
    ):
        raise StorageIdentityError("managed worktree repository changed")
    branch, tip = _worktree_branch_tip(worker)
...
def _worker_locator_value(primary_checkout: str, worker_checkout: str, task_id: str) -> dict:
    ...
    value = {
        ...
        "repository_key": parent["repository_key"],
        "checkout": worker,
        "primary_checkout": parent.get("primary_checkout") or os.path.realpath(primary_checkout),
        "home": parent["home"],
        "paths": paths,
    }
...
def bind_worker_locator(primary_checkout: str, worker_checkout: str, task_id: str) -> str:
...
def worker_locator_error(primary: str, worker: str, task_id: str) -> str | None:
    ...
    try:
        ...
        validate_task_worktree_registration(
            ...
            worker,
            task_id,
            target_commit=_git_value(worker, "rev-parse", "HEAD"),
...
def managed_write_allow(checkout: str) -> list[str] | None:
    ...
    if locator is None:
        return None
    return [os.path.join(path, "**") for path in sorted(locator["paths"].values())]
...
def submission_evidence_paths(checkout: str, step: str) -> list[str]:
    ...
    if step == "em":
        return [
            review_public_path(checkout, "findings.json"),
...
def external_store_root(workspace: str) -> str:
    ...
    root = _confined_stage_path(home, "projects", project_key(workspace), leaf_kind="directory")
    return root
...
def _remote_mode_file(workspace: str) -> str | None:
    ...
    if not url:
        return None
    h = hashlib.sha1(url.encode("utf-8"), usedforsecurity=False).hexdigest()[:16]
...
def _persistent_mode(workspace: str) -> dict:
    ...
    if private:
        return {"plan": plan, "store": "external", "private": True, "source": "private-setting"}
    ...
    if os.path.exists(shared_cfg):
        ...
        out = {"plan": plan or "team", "store": "repo", "private": False, "source": "shared-config"}
        return out
    if plan in ("team", "enterprise"):
        return {"plan": plan, "store": "repo", "private": False, "source": "plan"}
    return {"plan": plan or "personal", "store": "external", "private": False, "source": "default"}
...
def set_mode(workspace: str, plan: str | None = None, private: bool | None = None) -> dict:
    ...
    targets = [p for p in (_mode_file(workspace), _remote_mode_file(workspace)) if p]
    ...
    if targets and not wrote_any:
        ...
        raise OSError(f"could not persist taskplane mode to any of {targets}: {last_err}")
    persistent = _persistent_mode(workspace)
    if (
        persistent["store"] == "repo"
        ...
        try:
            ...
            with open(
                os.path.join(repo_store_root(workspace), "config.json"), "w", encoding="utf-8"
            ) as f:
                json.dump({"plan": persistent["plan"], "store": "repo"}, f, indent=2)
        except OSError:
            pass
...
def kb_root(workspace: str) -> str:
    ...
    return _confined_stage_path(store_root(workspace), "knowledge", leaf_kind="directory")
...
def _quarantine_shared_store_meta(path: str, workspace: str | None = None) -> str | None:
    ...
    if not os.path.lexists(path):
        return None
    quarantine = _confined_stage_path(
        store_home(workspace), "privacy-quarantine", leaf_kind="directory"
    )
...
def write_store_meta(workspace: str) -> dict:
    ...
    remote = (
        _run(["git", "config", "--get", "remote.origin.url"], cwd=workspace).stdout.strip() or None
    )
    shared = get_mode(workspace)["store"] == "repo"
    if shared:
        workspace_digest = hashlib.sha256(
            _workspace_identity(workspace).encode("utf-8")
        ).hexdigest()
    ...
    try:
        atomic_write_json(path, meta, indent=2, sort_keys=True)
    except OSError as exc:
        if shared:
            try:
                ...
                raise StateError(
                    path,
                    "shared store metadata write failed and stale raw "
                    "metadata could not be quarantined",
                    str(quarantine_error),
                ) from exc
            raise StateError(
                path,
                "shared store metadata write failed closed",
                ("stale raw metadata moved to private quarantine " + str(quarantined))
                if quarantined
                else "no shared metadata was published",
            ) from exc
        raise StateError(path, "private store metadata write failed", str(exc)) from exc
    return meta
